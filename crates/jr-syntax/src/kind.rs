...
pub enum SyntaxKind {
    ...
    /// consumer that handles one and forgets this one is a *missing arm* rather than a string comparison
    /// that falls through silently — the route by which `jr-fmt` has lost a construct in most of the last
    /// dozen waves.
    ///
