...
| **W4.5 — Pattern matching** | `switch` with exhaustiveness checking, a bare `.RED` as a case (ADR-0041 §2 step 5), and a **tagged** variant type beside `union` (ADR-0045 §1) | **Was missing from this table entirely.** Two accepted ADRs deferred decisions to it while no wave scheduled it — found while closing W2 (ADR-0054's handoff). Placed after W4 because exhaustiveness diagnostics want comptime type info, and before W5 because a polymorph over a variant type needs the variant to exist | 4–6 wks |
...
**The allocator protocol is real, and W3 has one feature left.** `context.allocator` is a procedure
pointer, `context.allocator_free` its release half, `context.allocator_data` the allocator's own
state — the placeholder ADR-0057 §1 put there "deliberately not usable as an allocator" is now what
it was named for. A program installs an allocator in one line and a callee allocates through it
without knowing which. **919 workspace tests**, all six gates green, plus **151 Neovim checks** that
are verified rather than gated. See §1.5 for component status.

**The wave was scoped by writing an allocator and seeing what the compiler refused**, not by reading
the handoff — and that found three gaps §7 had not recorded, each of which blocked the protocol:

- **A void-returning procedure pointer was unspellable.** `free: (*u8)` is what an allocator's
  release half looks like. `(*u8)` demanded `->`, `-> void` is E0212 (`void` has no type name), and
  `-> ` with nothing after it is a parse error. ADR-0062 §1 makes the arrow optional.
- **An imported `#foreign` procedure in a proc-pointer field reported two identical types** —
  *"expected `(s64) -> *u8`, found `(s64) -> *u8`"* — because they differ only in the invisible
  `ContextKind`. It is E0256 now, the code that says "wrap it". A *same-file* `#foreign` already gave
  E0256; only the imported path fell through, because the check read this file's HIR instead of the
  type.
- **`CONTEXT_FIELD_TYPES` is a `const &[PoolId]`**, so a context field's type must be a well-known
  id. The two proc-pointer types are pre-interned as `ALLOC_FN`/`FREE_FN`, which is why the fields
  are flattened into the context rather than nested in an `Allocator` struct.

**A corpus file was rewritten rather than extended, for the first time.** `046-context.jr` set
`context.allocator` to 5 and read it back; that field is a procedure pointer now. It tests the same
thing — that a context is one object reached by pointer — through `allocator_data`, and its header
says so, because a reader of that file would otherwise wonder what happened.
...
### What the allocator wave landed

- [x] **ADR-0062**: `context` grows to three fields; a void-returning proc-pointer type becomes
      spellable; an imported `#foreign` procedure in a proc-pointer position gets an actionable
      diagnostic. **No new diagnostic code** — §3 reuses E0256 and §1 removes a refusal, which is
      unusual enough to state.
- [x] **The default allocator is null, and calling it traps.** `main`'s context is zeroed (ADR-0057
      §5), so an uninstalled allocator is a null procedure pointer and calling through it traps
      rather than silently doing nothing. **Installing libc's allocator in the entry stub was
      rejected**: it would make `modules/Basic` a dependency of the *runtime*, which a `#c_call main`
      or a freestanding target could not satisfy.
- [x] **The formatter damaged source rather than losing it.** `") -> "` was emitted
      unconditionally, so a void-returning proc pointer came out as `(*u8) -> ` — which does not
      parse. `assert_parses` caught it; a survival assertion alone would not have. Tenth wave in
      twelve.
- [x] **The grammar's ambiguity became genuine.** `-> (s64)` now reads as both a results list and a
      void-returning proc pointer, with nothing after it to decide — so a declared GLR conflict, not
      a `prec`, which would silently pick one.

Two things worth carrying forward.

- **`type-errors/` files are checked with modules *unresolved*, on purpose**, so a refusal that needs
  an import must live in `imports/invalid/`. The E0256 file was written under `type-errors/` first
  and reported E0201 (unresolved `malloc`) instead — the directory contract catching a
  misfiled test, which is what it is for.
- **A well-known `PoolId` is the only way to name a type in a `const`**, and that shaped the
  context's representation more than any language argument did. Worth remembering the next time a
  compiler-declared type needs a field: the constraint is real and it pushes toward flat.

Diagnostic codes: **E0258 is the first free code**, **E0131 the first free *parser* code**. This wave
added **none**. The project defines **88** codes.

### Open, and honest about it

- [ ] **Temporary storage is W3's last feature**, and it wants this wave's allocator: a scratch arena
      the context points at, reset at a known point. It also wants `push_context` (below), because a
      scoped temporary allocator is exactly the form that needs restoring on scope exit.
- [ ] **`push_context` is absent** (ADR-0057 §6), so an allocator a callee installs stays installed
      for its caller. That is now a *visible* limitation rather than a theoretical one — a procedure
      that wants a scratch allocator for its own callees cannot restore the caller's afterwards.
      This is the next thing after temporary storage, or before it.
- [ ] **Traps with backtraces** remains the other W3 feature, and it is independent of everything
      above: a formatting trap helper (`TrapKind::reason()` is a `&'static str`) plus a call-stack
      representation in both engines.
- [ ] **Pointer arithmetic is still refused** (E0223), so a *bump* allocator is not writable — only a
      `malloc` wrapper. The corpus's `counting_alloc` counts bytes but still delegates to `malloc`.
      Temporary storage will want a real bump allocator, so this may become blocking rather than
      merely absent — worth deciding before that wave rather than during.
- [ ] **A `#c_call` proc-pointer type** is the general answer to §3's refusal, and needs a syntax for
      an attribute inside a type. Deferred (ADR-0062 §5).
...
- [ ] **`grammar.js` has no gate against loss** — gate 6 checks drift by regenerating, not reversion.
      Committing it between waves is the only guard, and thirteen waves are uncommitted.
...
- [ ] **Fourteen waves sit uncommitted**, ADR-0049 through ADR-0062, across five branches. The
      grammar loss two waves ago was the concrete cost; recorded again because the authorisation to
      commit is the decider's.
