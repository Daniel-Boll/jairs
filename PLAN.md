...
Accepted ADRs: 0001–**0218**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
> [!IMPORTANT]
> **ADR-0218 closes the missing enum-exhaustiveness code action.** E0258 now offers one preferred
> `add all missing cases` repair for ordinary `switch`, `if #complete`, enums and tagged variants.
> It inserts sema's declaration-ordered missing alternatives as explicit empty `case .NAME;` arms
> and deliberately offers no `else` shortcut, so a later enum addition remains visible.
>
> **The edit is current-source-safe.** The client may hold an older diagnostic, so the action
> requires the current E0258 range and missing-name list to agree before editing. It targets the
> switch's direct closing-brace token in the lossless CST, preserves CRLF and project indentation,
> and converts the byte position through the negotiated UTF-8/UTF-16 map.
>
> **No compiler rule changed.** Alias value classes and tagged-variant case sets remain the
> compiler's judgement; the editor consumes them. This wave adds no corpus file and needs no LLVM
> gate for its implementation surface.

**1290 workspace tests (1301 under gate 7), 291 corpus files, 218 ADRs, 25 modules, all six required
gates and gate 7 green.** ADR-0218 adds ten LSP tests and no corpus file, because its deliverable is
an editor edit over the already-covered E0258 semantic rule. **E0297** is the first free global
