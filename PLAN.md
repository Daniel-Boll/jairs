...
**W5 — Polymorphism is OPEN, nine sub-waves done.** `$T` procedures, polymorphic structs and `$N`
comptime-value parameters are **complete**; `#expand` macros **work** — a call splices the body into the
caller's scope. **979 workspace tests**, all six gates green, **166 Neovim checks**. See §1.5.
...
Diagnostic codes: **E0274 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
...
lowering builds the splice). The project defines **105** codes.

### The next wave

**`#modify` next, then `#bake_arguments` — the last of W5.** The `#expand` splice works (ADR-0091), so what
remains of the macro family is the two refinements *of* a macro: `#modify` runs code at compile time to
