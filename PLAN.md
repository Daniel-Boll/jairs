...
Diagnostic codes: **E0275 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
...
lowering builds the splice). W5 also added E0274 (a call to a `#modify`
procedure, refused pending evaluation). The project defines **106** codes.

### The next wave

**`#modify` next, then `#bake_arguments` — the last of W5, and `#modify` is now unblocked.** Designing it
surfaced that its enabling piece was missing — `type_info(T)` on a bound variable was E0261 — which
ADR-0092 fixed, so a predicate can now ask `type_info(T).id == type_info(s64).id`. What remains of the macro
family is the two refinements *of* a macro: `#modify` runs code at compile time to
