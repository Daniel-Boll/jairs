...
**W6 — Metaprogram is OPEN.** Sub-wave 1 (`@note`) is green: a declaration can carry `@name` or
`@name "payload"` metadata for a metaprogram to read. **981 workspace tests** and **181 corpus files**, all six gates green, **166 Neovim checks**. See §1.5.
...
**Notes ship before their reader deliberately.** ADR-0080 §3's rule — represent a thing only once something
can inspect it — cuts the other way for *input*: the message loop's job is to hand declarations to a build
script, and a declaration with nothing extra to say is not worth handing over. Designing the loop first would
mean designing its message shape against no consumer.

**What W6 has left, in order:**
- [ ] **The compiler message loop** — a `#run`-time way for a metaprogram to *ask* for the declarations the
      compiler has seen, so `@note` gains a reader. The first fork to settle: whether the loop is a genuine
      poll (`compiler_wait_for_message()`) or a callback the compiler invokes, and what a "message" is as a
      Jairs value — which is ADR-0080 §3's `Code`-value question in a new place.
...
**W6 — Metaprogram is next, then W7 — Stdlib.** W5 is complete: `$T` with multiple variables and nested
inference, polymorphic structs, `$N` including `[N]T`, `type_info(T)` on a bound variable, `#expand` macros
that splice, `#modify` predicates that run and reject, and `#bake_arguments` specialisation — fifteen
sub-waves, ADR-0081 through ADR-0097. W6 is workspaces, the compiler message loop, and `#run build()` build
scripts replacing makefiles (PLAN §2.1); W7 is the stdlib in Jairs, whose `Array`/hash table need exactly the
polymorphic structs W5 just delivered. Original note on premise-checking, still the rule: `#modify` runs code at compile time to
inspect or reject an instantiation, and `#bake_arguments` produces a specialised procedure from a partial
application. Both now have something to refine. Also owed from 7b, each its own decision: the
**caller-return** semantics for an early `return` (E0273 refuses it today) and a **cross-file** splice
(E0272). Original note on premise-checking, still the rule: `$T`, polymorphic structs and `$N` (surface, instantiation, `[N]T`)
