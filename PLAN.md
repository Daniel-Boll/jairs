...
**W5 — Polymorphism is OPEN, fourteen sub-waves done.** `$T` procedures, polymorphic structs and `$N`
...
- [ ] **`#bake_arguments` specialisation** — produce the clone and lift E0276. **The last of W5.** Its
      mechanism is settled (ADR-0096 §1): reuse `append_one`'s three steps — drop the baked parameters,
      substitute their values, remap the remaining `Res::Param` indices — with the values coming from
      ADR-0088 §2's const-eval pre-pass;
...
**`#bake_arguments`'s specialisation is the last of W5.** `#expand` and `#modify` are complete and
`#bake_arguments` has its surface, so one step remains — and its mechanism is settled (ADR-0096 §1: reuse
ADR-0088 §3's clone). After that, W6 Metaprogram and W7 Stdlib, in that order. Original note on
premise-checking, still the rule: `#modify` runs code at compile time to
