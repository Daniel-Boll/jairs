...
**Traps report a call chain, and W3 is complete.** A trap now prints the procedure frames that were
live beneath it, innermost first, and **both engines produce byte-identical bytes** — the VM by pushing
a `ProcRef` in `Vm::call` and resolving names from the HIR, native by writing name pointers into a
mutable global that its generated helper walks at trap time. That was the last feature W3 asked for
(§2.1's "panics/traps with backtraces"), so **wave W3 — Runtime core is done**. **924 workspace tests**,
all six gates green, plus **156 Neovim checks** verified rather than gated. See §1.5.

**The wave's scope was set by five constraints, each verified rather than assumed**, and they are worth
carrying because they bound what any future backtrace work can do:

- native **embeds a fixed message string per trap site at compile time** — a linked binary has no source
  map to consult;
- there is **no runtime object at all** (`jr-link`'s docs: the helper is generated into the object), so
  no unwinder and no symbol table exist for a stack walk;
- the differential harness compares a trapping program's stderr **byte for byte**, so both engines must
  produce *identical* output — ruling out "native uses the platform unwinder, the VM uses its frames";
- **inlining erases callee frames on purpose** (ADR-0021 §3 rewrites every copied span to the call site,
  because a callee `MirSpan` names the callee file's arenas), and both engines run `optimized_file_mir`;
- **neither engine recorded who called whom** — a four-deep *recursive* chain, which the inliner cannot
  flatten, still printed one line.

So a source-level backtrace with inlined frames would need an inline-provenance field on every
`MirSpan` that every pass must maintain, replacing ADR-0021 §3's *structural* guarantee with a
discipline no verifier can check. ADR-0066 §4 rejects that and takes the shadow stack instead.
...
### What the backtrace wave landed

- [x] **ADR-0066**: a shadow call stack in both engines, rendered by `jr_base::trap_message` as one
      `  in <name>` line per frame. **No new diagnostic code** — a backtrace is a runtime message, not a
      diagnostic — so **E0258 is still the first free code**.
- [x] **A wrong claim in this ADR's own draft, caught by running it.** §4 predicted that in
      `inner → middle → main` both callees would be inlined away, leaving only `main`. Running it printed
      `in middle` / `in main`: `is_inlinable` refuses any callee that *makes a call of its own*
      (ADR-0021 §4's caller-side exclusion), so only leaves inline — `inner` vanished, `middle` did not.
      The ADR now records what happens rather than what was assumed.
- [x] **A real asymmetry between the engines, found by the harness.** `main`'s frame is pushed by its
      *caller*, and native's caller is the C entry shim while the VM's is `run_main` — so native printed
      one frame fewer until the shim learned to push it. Pinned by its own test.
- [x] **The backtrace independently confirms the inlining claim.** The two ADR-0021 differential tests
      asserted a *line number* to show a callee was or was not inlined; they now also assert the chain,
      so `in bump` appearing exactly when `bump` was not inlined is a second, structural check of the
      same property.

Two things worth carrying forward.

- **An ADR written before running the thing can be wrong about its own example**, and this wave's was.
  The forks were sound; the illustration was not. Running the example *while* writing the ADR — not
  after — is what keeps the record honest, and it is cheap.
- **A feature can be bounded honestly instead of faked.** Inlined frames and indirect-call frames do not
  appear, because at run time they did not exist. Both absences are stated in ADR-0066 §4/§5 rather than
  papered over with reconstructed names, on the same principle ADR-0020 §4 used for locations: reporting
  nothing beats reporting a neighbour.
...
- [ ] **W4 — Comptime is the next wave, and §2.1 calls it the hardest**: full `#run` over arbitrary
