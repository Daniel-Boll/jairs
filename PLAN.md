...
Accepted ADRs: 0001–**0212**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
> [!IMPORTANT]
> **ADR-0212 closes a formatter-induced ABI break found by the optimisation audit.**
> `jr fmt` deleted `#program_export`; the result still parsed and type-checked but codegen mangled
> the symbol, so a C caller could no longer link the library. The attribute now survives in source
> order and a regression pins both legal orders beside `#c_call`.
>
> **This is an emergency correction, not the formatter redesign.** The audit found duplicated raw
> node-kind allowlists and wildcard fallbacks throughout `jr-fmt`; typed exhaustive dispatch is the
> recommended structural follow-up. LSP diagnostic freshness still awaits the decider's selection
> between all-open batching, dependency-closure scans and a reverse import graph.
>
...
**1229 workspace tests (1238 under gate 7), 283 corpus files, 212 ADRs, 25 modules, all seven gates
green.** ADR-0212 adds one formatter regression and no corpus file or module.
