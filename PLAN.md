...
Accepted ADRs: 0001–0178. See [`docs/adr/README.md`](docs/adr/README.md). (This line
...
`--> file:line:col` come from one resolution. That is ADR-0020 §2's one-formatter rule applied to two consumers
rather than two engines, and it is what stops a line table saying 41 while the trap says 40.

**Item 2 is done for LLVM** — ADR-0171: a struct's layout reaches DWARF as a `DW_TAG_structure_type` with
source field names and the offsets `jr_pool::field_offset` computes, which is the *same* function both engines
use to compile a field access, so a debugger cannot disagree with the code about where `p.y` is.

> [!IMPORTANT]
> **Items 2 and 3 turn out to be coupled, and this section had them as separate lines.** The struct mapping was
> written, it was correct, and `dwarfdump` showed base types and **no struct** — because **LLVM prunes a type
> nothing declares**, and a `DISubroutineType` is a signature rather than a declaration. What retains a type is
> a *variable* of it. So each parameter now gets a `DILocalVariable`, and a plan that budgets "type DIEs" and
> "locals" independently is wrong about both: a type DIE with nothing declaring it is not emitted, and a
> declared parameter with no type DIE has nothing to point at.

**Item 3 is half done for LLVM** — ADR-0172: an **escaped scalar** local reaches DWARF with its source name, its
type and a stack location, so `lldb` can print it. The other half is now *specified* rather than vague, which is
the wave's real output:

> [!IMPORTANT]
> **Two boundaries, both found by writing the test rather than by planning.**
>
> **A register-resident local is invisible.** Only an *escaped* local gets a MIR slot (ADR-0017 §2), and a slot
> is what a `dbg.declare` describes — a local living entirely in SSA registers has no address to point at. That
> is exactly what "locals through value labels" is for, and it is a *different DWARF expression* rather than a
> missing call.
>
> **An aggregate local is unnamed**, even though it escapes: its slot carries no `LocalId`, so nothing connects
> it to a source name. A MIR-side gap, not an emission one.
>
> Both are **asserted** — the second negatively, with a message telling whoever fixes MIR to invert the line.
> An absence that is asserted is a boundary; an absence merely omitted is something the next reader
> rediscovers.

**Also worth knowing before touching LLVM debug info again**: LLVM 19 replaced the `llvm.dbg.declare` *intrinsic
call* with a debug **record**, and inkwell 0.9 casts the returned record to a value and wraps it in
`InstructionValue::new`, which asserts. **Both** of its insert helpers panic on LLVM 21, at a message naming
inkwell's internals and no call of ours. The raw `llvm-sys` `LLVMDIBuilderInsertDeclareRecordAtEnd` is used
instead.

**Cranelift now has a `.debug_info` too** — ADR-0173: base, pointer and struct type DIEs with named members at
real offsets, and a subprogram per function carrying its source name, address and length. **So both back ends
agree about a struct's layout in DWARF, by two entirely different routes** — one is metadata LLVM writes, the
other is bytes this project writes with `gimli`, and the test asserts the agreement rather than each emitter's
existence.

> [!NOTE]
> **That was the prerequisite this section never listed.** "Locals through Cranelift value labels" needs a
> variable DIE, a variable DIE needs a subprogram to live in and a type to point at, and Cranelift had neither —
> only a line program pointing into a `.debug_info` that was not there. ADR-0173 §4 now specifies the remainder
> exactly: a **stack** local wants `DW_OP_fbreg` plus a per-slot frame offset (`MachBufferFinalized::frame_layout`
> exists; whether it exposes per-slot offsets is the next thing to *probe*), and a **register** local wants
...
**And the event loop works too** — ADR-0165, which **amends ADR-0164 §5 by contradicting it.** That section
claimed no event loop was possible, because `SDL_Event` is a union and E0286 refuses one at a `#foreign`
boundary. The refusal is right and **irrelevant**: E0286 refuses an aggregate crossing *by value*, and
`SDL_PollEvent` takes a **pointer** — the same shape as the `*Rect` that module had been passing all along.

> [!IMPORTANT]
> **This is the clearest instance yet of the habit §7 closes with**, and the first where it caught an *accepted
> ADR of this project's own*, written in the same session, rather than an assumption inherited from a plan.
> ADR-0164 §5 planned around a premise it never wrote, then built a story on it — "four waves at one boundary",
> and a claim that settling this fork also settles the Objective-C question. **Both are withdrawn.** The
> correction cost one probe: a 56-byte `#place` overlay, a `*Event` argument, push a synthetic `SDL_QUIT`, poll
> it back. Four assertions, four passes, **no compiler change of any kind**.
>
> An ADR is evidence of a decision, not evidence of a fact.

`#place` (ADR-0144) turns out to *be* the union mechanism — two fields at one offset is what a union is — so
`key_sym` and `mouse_x` genuinely share offset 20 and the test asserts the sharing rather than tolerating it.
Two smaller findings came out of writing it: **SDL does not promise one-push-one-poll** (a one-poll test passed
on the first push and failed on the second, so `wants_to_close` drains), and **a synthetic `KEY_DOWN` is pushed
successfully and then dropped by SDL**, so the keyboard assertions read a locally-built event instead.
...
grammar *rule*. **1071 workspace tests** (1075 under gate 7) and **255 corpus files**, all six gates green **locally**, plus the new **gate 7** (the LLVM back end, which needs an installed LLVM 21) — no CI run
...
- [ ] **A field of a call's *result* has no place** (ADR-0156 §5): `mk().count` where `mk` returns a
      `string` is refused as "a memory reference has no place", E0245, so a legal program does not lower.
      MIR needs a *place* to read a field from and a call produces a *value*, so the fix is to materialise
      the result into a temporary slot — the same move ADR-0076 §1 makes for an `Any` and ADR-0139 §2 makes
      for a variadic pack, which is why this is small rather than structural. The workaround is a named
