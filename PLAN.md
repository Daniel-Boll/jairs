...
Accepted ADRs: 0001–0142. See [`docs/adr/README.md`](docs/adr/README.md). (This line
...
**W8 — Performance is OPEN**, one sub-wave in (ADR-0142, the optimisation level). **W7 — Stdlib is
...
worked. **1018 workspace tests** and **228 corpus files**, all six gates green **locally** — no CI run
...
- [ ] **The LLVM back end via `inkwell`**, and the three-way differential that is the point of having it
      (ADR-0019 §5 assigned `jr-codegen-llvm` here; the crate is an empty `lib.rs`). Forks to settle before
      code: whether LLVM is a **cargo feature** (default off, so the six gates do not require an LLVM
      installation) or an unconditional dependency; which LLVM version the workspace pins, since `inkwell`
      names it in a feature and a mismatch is a link error rather than a version warning; and what selects
      a back end on the command line — `-O` reaching the back end is named in ADR-0142's consequences as
      this sub-wave's business.
- [ ] **Inliner maturity**, which §1.5 already itemises: **cross-block store-to-load forwarding** and
      **SROA**, plus compacting the SSA value arena. Each is a pass, so each is a candidate for the `-O2`
      that ADR-0142 §1 refused to invent — the first one whose cost justifies opting in gets the level.
- [ ] **`#align` and `#place`**, which are layout control and therefore touch `jr-pool`'s `layout_of` — the
      one place layout is computed (ADR-0018 §2), which is what makes them tractable.
- [ ] **`#soa`** and **SIMD vectors**, both of which need a decision about whether they are types or
...
**W8 — Performance is now the wave being worked**, and its own section above tracks it. It went first
because it depends only on W5, and because its first sub-wave (ADR-0142) buys a diagnostic tool the two
other open waves will both use: `-O0` makes a wrong answer attributable to lowering rather than to a
pass. Still open beside it: W6's remaining metaprogram work (the static-data table for the message loop),
W7's remaining modules (merge sort with an allocation policy, `String.split`, `File`, etc. — the merge
sort is *owed to W8* by ADR-0104 §3, with a benchmark behind it), and the two security dispatches above.
