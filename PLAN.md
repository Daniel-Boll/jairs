...
**W6 — Metaprogram is OPEN**, two sub-waves in. A declaration can carry `@name`/`@name "payload"` metadata
(ADR-0098) **and a metaprogram can now read it** — `has_note(f, "x")` and `note_value(f, "x")`, folded at
compile time with no VM (ADR-0099). **981 workspace tests** and **185 corpus files**, all six gates green, **166 Neovim checks**. See §1.5.
...
- [ ] **The compiler message loop** — a `#run`-time way for a metaprogram to *iterate* the declarations the
      compiler has seen, so a build script can ask "every declaration tagged `@X`" rather than naming each.
      ADR-0099 built the *reading verbs* the loop's messages will be handed to; what remains is the iteration
      itself. The first fork to settle: whether the loop is a genuine poll (`compiler_wait_for_message()`) or a
      callback the compiler invokes, and what a "message" is as a Jairs value — which is ADR-0080 §3's
