...
**Temporary storage exists, and W3's data structures are complete.** `talloc(n)` hands out `n` bytes
from a per-context bump arena that stays valid until `reset_temporary_storage()`, no per-piece free. It
is the last W3 data structure, and it *assembles the previous three waves rather than adding
machinery*: a region from `malloc` (ADR-0060), a cursor advanced with pointer arithmetic (ADR-0064),
and two context fields to hold them (ADR-0057). **919 workspace tests**, all six gates green, plus
**156 Neovim checks** verified rather than gated. See §1.5.

**The wave was scoped by running the building blocks first.** A program that `malloc`s a region and
bumps a cursor through it with `p + off` ran in both engines before any design — so temporary storage
was known to be a composition, not new machinery, and the wave's whole compiler change is two context
fields. `talloc` and `reset` are ordinary Basic procedures.

**Nothing new lowered, and no new well-known id.** `temp_data` (`*u8`) and `temp_mark` (`s64`) are both
already well-known pool ids, so `Context` grows to five fields with no pre-interning and
`WELL_KNOWN_COUNT` unchanged. The basic-module snapshot shows `talloc` reading `(v0).*.3` and `(v0).*.4`
— the new fields — and bumping the cursor with pointer arithmetic. No new MIR node, no back-end change,
...
### What the temporary-storage wave landed

- [x] **ADR-0065**: `talloc`/`reset_temporary_storage`, a lazily-`malloc`'d fixed 64 KiB bump arena in
      two context fields. **No new pool id** (both field types already well-known), **no new MIR node**,
      **no new diagnostic code** — E0258 still first free. The whole compiler change is two entries in
      `CONTEXT_FIELD_TYPES`/`_NAMES`.
- [x] **It lives in Basic, not the language** — the *opposite* call from ADR-0062 §5, which kept the
      allocator *protocol* out of Basic. The protocol is a language mechanism (how a callee reaches its
      caller's allocator); temporary storage is a *concrete* allocator with one policy, which is what a
      library provides. It still travels with the context, because it reads `context.temp_*`.
- [x] **A corpus type-error the composition exposed**: `talloc` returns `*u8`, so a callee storing a
      value through it stores a `u8`, and the first draft stored an `s64` (E0214). A `*s64` would need a
      pointer cast the language does not have (ADR-0046 §2), so the corpus stores bytes — the shape
      available now, and honest about it.
- [x] **The region is lazy and fixed.** Null until first `talloc`, so a program that never uses it pays
      nothing and the entry stub stays free of `malloc` (ADR-0062 §4's argument). Fixed rather than
      growable because a moving `realloc` would invalidate every pointer already handed out; overflow
      returns null like `malloc` rather than trapping (ADR-0065 §3).

Two things worth carrying forward.

- **A feature that adds no machinery is the payoff of the ones that did.** Temporary storage is three
  prior waves composed — the compiler change is two context fields, and everything else is Basic code.
  That is what "the allocator, then pointer arithmetic, then this" was building toward, and it is worth
  noticing when a wave costs almost nothing because the groundwork was laid.
- **`talloc`'s `*u8` return is a real limit**, not a stylistic one: without a pointer cast, an arena can
  only hand out byte buffers, so storing a wider type needs the cast a later wave owes (ADR-0046 §2).
  The corpus stores bytes and says so, rather than faking a wider store.
...
- [ ] **W3's remaining feature is traps with backtraces**, and it is independent of everything shipped:
      a formatting trap helper (`TrapKind::reason()` is a `&'static str`) plus a call-stack
      representation in both engines. Its native half is the hard part (a stack walk entangled with the
      inliner). With temporary storage done, this is the last thing between here and W3 complete.
