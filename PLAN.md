...
**W4 — Comptime is open, split into sub-waves, and its first one is done.** A `#run` may now call an
**imported** procedure and appear inside a **body**. **930 workspace tests**, all six gates green, plus
**166 Neovim checks** verified rather than gated. See §1.5.

**W4 is delivered as a sequence, and that is a decision rather than a convenience** (ADR-0069 §0). §2.1
gives it six deliverables and 10–14 weeks; every other wave here has been one ADR and one branch. A wave
five times that size cannot be verified the way the others were — the handoff at the end would be a claim
nobody could re-run in a sitting, which is exactly how §7 rots. The sub-waves:

1. **`#run` across files and in a body** — done, ADR-0069.
2. **Aggressive const folding** — folding a `#run` result into the MIR that consumes it. **Next.**
...
**§7's own claim about `#run` was wrong in both directions, and running is what showed it.** For several
waves this section said the compiler had "one *trivial* `#run`: a call or a constant expression, same file
only". Two of those three qualifiers were false: nested calls, arithmetic around a call and a `while` loop
in the callee all already evaluated. The handoff was *underselling* — the mirror of the rot this section
warns about, and worth naming because a handoff that undersells is as untrustworthy as one that oversells.
What genuinely failed was cross-file (with an **internal compiler error**, not a refusal) and in-body.
...
### What the comptime-run wave landed

- [x] **ADR-0069**: every reachable file's bytecode in the comptime program, and a body `#run` collected
      as a const-eval target. **No new diagnostic code** — §1 removes a failure and §2 lifts a refusal, so
      **E0261 is still the first free code**.
- [x] **Two internal compiler errors became actionable.** `no routine for file 1 proc 11` is now a working
      program; a `#run` that omits a defaulted argument now says so instead of leaking the interpreter's
      "called a procedure taking 3 arguments with 2". The second was pre-existing at file scope and only
      *newly reachable* from a body — found because the new position exercised it.
- [x] **The ADR's own §1 was wrong and is corrected in place.** It claimed supplying routines "adds no
      dependency that was not already there"; taking the MIR from `file_mir` produced a salsa cycle and
      three corpus tests failed immediately. The imported file's MIR is lowered inside `file_consts`
      instead, from queries it already calls — which is the honest position, not a workaround.
- [x] **A test whose construct I had to replace for the second time.** `a_refused_body_is_a_diagnostic`
      used a body `#run` because it "stays refused for several waves yet"; this wave made it work, one
      wave later — exactly as its predecessor comment had been falsified by ADR-0055. It now uses a
      directive in a body, refused *by design* (ADR-0016 §3).
- [x] **A test of mine that passed vacuously.** The first body-`#run` freeze test asserted `unchanged` on
      a *leaf* callee — nothing inlines into a body with no calls, so it passed with the whole feature
      disabled. Rewritten so the callee has a call of its own, then teeth-checked.

Two things worth carrying forward.

- **A test that names an unimplemented thing has a one-wave shelf life.** This is now twice for the same
  test. If a test needs a refused construct, name one refused *by design* — every unimplemented thing is
  one wave from working, and the test's real subject is usually the crash, not the gap.
- **An over-approximation that is cheap in one arena can be catastrophic in another.** `const_callees`
  walked whole expression arenas, which costs "a procedure or two of missed inlining" at file scope — a
  file arena holds only file-level expressions. A *body's* arena holds everything, so the same walk froze
  almost every procedure and disabled the bounds-check strip. The roots are found first now.

Diagnostic codes: **E0261 is the first free code**, **E0131 the first free *parser* code**. This wave
added **none**. The project defines **91** codes.


### Open, and honest about it

- [ ] **W4 sub-wave 2 is aggressive const folding**, and it is the next thing: a `#run` result reaches a
      body as a constant now, but the arithmetic around it is not re-folded (ADR-0069 §4). Doing it in the
      same wave would have meant the corpus could not distinguish "the value arrived" from "the value
      arrived and was folded".
- [ ] **W4's remaining sub-waves are RTTI and `#insert`/`Code`**, and §2.1 calls the wave the hardest: full `#run` over arbitrary
