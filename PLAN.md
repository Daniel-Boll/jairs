...
waiting. Both are tracked below. **990 workspace tests** and **213 corpus files**, all six gates green, **166
Neovim checks**. See §1.5.

> [!IMPORTANT]
> **An audit was run at `354d900`** and is recorded in
> [`docs/assessment-2026-08-07.md`](docs/assessment-2026-08-07.md). Sub-wave 18 (ADR-0120) closed its one
> Critical finding; sub-wave 19 (ADR-0121) closed the unbounded-compile one. Its remaining findings are the
> next work, in this order: **`BUILD_OUTPUT` is unconstrained** (a program names an absolute path and
> `jr build` writes an executable there, plus argument injection into `cc`), **no cross-crate diagnostic-code
> uniqueness check** (`jr-hir` and `jr-db` have no `code.rs` at all), **the attribute token-set trap** (two
> unlinked literal lists, seven bugs and counting), and **the docs' own drift** — including that `README.md`
> claims Linux "is kept green in CI" when **CI has never run once**.
