...
**W5 — Polymorphism is OPEN, five sub-waves plus a sixth's first half done.** `$T` procedures and
polymorphic structs work; `$N` comptime-value parameters have their *surface* (parse, lower, body checks),
with a call refused (E0271) pending instantiation. **976 workspace tests**, all six gates green,
...
- [x] **`$N` comptime-value parameter — surface** (ADR-0087, sub-wave 6a): `$N: s64` parses, lowers,
      ...
      only its value varies, so `N + true` is E0214 at template time). A call is refused by design (E0271)
      pending the second half. `ProcSig::is_template` now covers both the `$T` and `$N` marks, keying the
...
Diagnostic codes: **E0272 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
...
and E0271 for a call to a `$N` comptime-value-parameterised procedure (refused pending its instantiation
half). The project defines **103** codes.

### The next wave

**`$N` instantiation next, then macros.** The surface (6a) is done; the second half evaluates the argument
and makes a call run. Before starting it, confirm the premise the way the surface's was confirmed (the habit
has caught a false schedule *five* times — ADR-0067 §0, ADR-0070 §0, ADR-0072 §5, ADR-0073 §0, and
ADR-0075's own closing claim): in particular, verify the const-eval-at-a-call-site path is genuinely the
same `insert_operands`/`file_consts` machinery W4 built and not a second evaluator, and that keying an
instantiation on argument *values* composes with the existing `instantiate.rs` clone (which keys on types).
The array case `[N]T` is where it meets polymorphic structs. Macros are the larger family after it, wanting
`#expand` to compose with `#insert`/`#code`.
