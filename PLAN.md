...
**W6 — Metaprogram is OPEN**, three sub-waves in. A declaration can carry `@name`/`@name "payload"` metadata
(ADR-0098), a metaprogram can **read** it — `has_note(f, "x")`, `note_value(f, "x")` (ADR-0099) — and can now
**query** it: `noted_count("x")` and `noted_name("x", i)` find the declarations carrying a note without
naming them (ADR-0100). All four fold at compile time with no VM and no new query. **981 workspace tests** and **187 corpus files**, all six gates green, **166 Neovim checks**. See §1.5.
...
- [ ] **Static data, then the loop.** The message loop's remaining scope is now *purely the iteration
