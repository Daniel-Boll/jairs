...
**W4 — Comptime is COMPLETE as scoped.** Sub-wave 10 closed it with `#code` (ADR-0080) and two honest
refusals: a pointer or view in a compile-time aggregate (ADR-0079, a *shipped silent miscompile*), and a
clear diagnostic where a `#run` reads an imported constant. **969 workspace tests**, all six gates green,
**166 Neovim checks**. See §1.5.

**What W4 delivered, in ten sub-waves:** `#run` across files and in a body (0069); an array length from a
constant (0070); a type as a compile-time value (0071); `#insert` of a literal (0072) then a computed
(0073) operand; aggregate constants (0074); `type_info` and a constant holding a string (0075); `Type_Info`'s
stable `id` (0077); `Any` with a checked read plus its `*T` coercion (0076); `Type_Info`'s fixed-size
per-kind facts (0078); and `#code` (0080).

**Three things are deliberately *not* in it, each with its reason recorded rather than deferred vaguely:**
...
Diagnostic codes: **E0268 is the first free code**, **E0132 the first free *parser* code**. Sub-wave 10 added
**one** — E0131, `#code` with no braced body. The project defines **99** codes.

### The next wave

**W5 by §2.1's order.** Before starting it, check its stated premise against what exists — that habit has
now caught a false schedule *five* times (ADR-0067 §0, ADR-0070 §0, ADR-0072 §5, ADR-0073 §0, and ADR-0075's
own closing claim, which ADR-0078 had to correct). The field list above is the one piece of W4's *subject*
left, and it is a wave rather than a remainder.
