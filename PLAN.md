...
wrong pointee type a silent wrong read — the reinterpretation ADR-0045 §1 confined to `union`. Nothing is
reinterpreted, because a pointer's bits do not depend on its pointee, so the erasure and its inverse each
go through a slot and emit no code.

**`Type_Info` gained a stable `id`** (ADR-0077, amending ADR-0075 §3), and the reason is the sharpest kind
this project keeps finding: `any_as` needs a runtime type identity, and probing proved the four-field
schema had none a *sound* check could use. Two `type_info(T)` calls have different addresses, so pointer
comparison is out; `size`/`alignment` collide; and `name` is unsound because nominal identity is a
declaration site, not a spelling (ADR-0015 §1). `id` is the pool id — the identity the compiler already
uses, identical in both engines because they share one pool — so `any_as` is an integer compare and a
conditional trap.

### What the wave landed

- [x] **`any_of` and `any_as` as intrinsics**, recognised by name and only when unresolved (a program may
      declare its own), lowering to real MIR rather than folding to a constant like `type_info`. `any_of`
      builds the `{type, data}` aggregate; `any_as` loads `a.type.id`, compares, and traps on mismatch —
      ADR-0068's tagged-read rule one level up.
- [x] **`Type_Info.id`, and the validation moved with it.** `TYPE_INFO_FIELDS` gained `("id", s64)` at the
      front, so a `Basic` whose `Type_Info` lacks it is E0265. This is the first exercise of ADR-0075 §3's
      "the shape extends by adding fields", and it *does* shift the four's offsets — which is exactly why
      nothing reads a field by a hard-coded offset and `field_offset` computes them.
- [x] **`Any` declared in `Basic`, validated on lookup** — the second client of ADR-0075 §2's mechanism,
      and the first evidence it generalises. `report_library_shape` names which type is wrong, because
      there are two now.
- [x] **`field_place` spills an aggregate-valued receiver with no place into a slot.** `type_info(s64).id`
      reads a field of a by-value result — a value, not a place — and refused with "a memory reference has
      no place". The spill is ADR-0075 §2's `Res::Item` move, generalised to a call result. Scalars with
      no place stay a real refusal.
- [x] **The dump masks a `Type_Info`'s `id` as `#id`.** The value is a pool index, which load order
      renumbers — the snapshot churn `AGENTS.md` forbids, the same as the `FileId` and bare-`enum` cases.
      Detected by field-type shape, since the dump holds no interner.
- [x] **Teeth-checked**: bypassing the `id` comparison makes the mismatch differential fail (the read stops
      trapping). Both engines exit 4 on a mismatch and agree byte-for-byte.

Diagnostic codes: **E0268 is the first free code**, **E0131 the first free *parser* code**. This wave added
**one** — E0267 (`any_of` of a non-pointer). The project defines **98** codes.

### Open, and honest about it

- [ ] **Per-kind detail in `Type_Info`** (ADR-0075 §3) — a struct's field list, an array's element type, a
      procedure's signature. Each is a *variable-length* member wanting a view or a pointer-and-count, so it
...
- [ ] **A `#run` reading another file's constant** stays refused (ADR-0073 §4): the narrow acyclic pre-pass
      is precisely what does not deliver a general cross-file value dependency. It needs its own decision.
