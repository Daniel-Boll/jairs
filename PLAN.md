...
> [!IMPORTANT]
> **`Simp` and `Window` now have Jai's real API, over OpenGL** — ADR-0185 to ADR-0188. Every signature
> came from copies of Jai's own module source vendored verbatim in two open-source projects and **diffed
> against each other**, not from documentation. Eight were wrong, and two not cosmetically: the
> coordinate origin was mirrored and every call carried a state handle Jai does not have.
>
> Getting there needed **file-scope mutable variables** (ADR-0186) in all three engines, because that
> handle existed only to stand in for state the language could not hold. Jai keeps the equivalent in
> `#add_context simp: *Immediate_State` plus one process-wide `the_gl_context`.
...
Tests **1076 → 1082**; the workspace corpus **266 → 269** `.jr` files under `tests/corpus/` outside
`tests/corpus/modules/`. ADRs 184 → **188**. **No new diagnostic code** — E0295 is still the first free
one, which is worth noting for a stretch this size: four ADRs and not one new refusal, because every
defect here was a *gap* to close rather than a construct to refuse.
...
**Newly owed, from ADR-0185/0188, ranked by what real Jai code actually uses:**

1. **`T.[a, b, c]` array literals — 39 occurrences** in the three repositories read, the single most
   used construct Jairs lacks. Parser, HIR, sema and a MIR aggregate constant.
...
3. **A typed constant.** Third stretch running, and it now has a fresh count: every one of `modules/GL`'s
   ~20 `GLenum` constants is `cast(u32, X)` at each use, because `X : u32 : 5` does not parse. Still the
   highest-value *small* item.
4. **`type_of(x)` — 14 occurrences.** Jairs has `type_info`, so this is a small addition beside it.
...
7. **Cross-file globals.** Reading an imported module's global directly is E0245. A module's own
   procedures reading its own global work across the boundary, which is the whole `Simp` use case, so
   this is not urgent — and it is the `ImportedProcs` pattern again when something needs it.
