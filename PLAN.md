...
**`push_context` exists, and it makes an isolation claim true that was false for six waves.** A
`push_context { … }` block gets its own copy of the context, so a write to `context.allocator` inside
it — or by anything it calls — is discarded on exit and the enclosing scope's context is restored.
ADR-0057 §2 had *claimed* this ("a caller's context is not affected by its callee's writes… each
procedure modifies a copy it owns") and it was never true: the context is one object reached by
pointer, so writes leak both ways, and `050-allocator.jr` *relies* on the upward leak. ADR-0063
amends §2 to say so and adds `push_context` as the one construct that really isolates. **919 workspace
tests**, all six gates green, plus **156 Neovim checks** verified rather than gated. See §1.5.

**The wave was scoped by running the claim, not trusting it.** Before writing anything, a two-line
program set `context.allocator_data` in a caller, had a callee overwrite it, and checked whether the
caller saw the change. Both engines said it did — so §2's isolation half was false, and the wave's
framing became "make it true where a program asks" rather than "build on §2". That is the "verify by
running" rule catching a documented invariant that the implementation never had.

**No new MIR node, no back-end change.** `push_context` lowers to an aggregate copy into a fresh
`Context` slot and a compile-time swap of which `*Context` operand `context` reads (ADR-0063 §2) — the
same `Load`/`Store` that lowers `b := a`, which both engines already memcpy. The MIR snapshot of
`valid/051` shows exactly that: `s0: Context`, `load (v0).*`, `store s0`, `addr s0`.
...
### What the push_context wave landed

- [x] **ADR-0063**: `push_context { … }` copies the context on entry and restores it on exit; amends
      ADR-0057 §2, whose isolation claim was false. **No new diagnostic code** — a `push_context` in a
      `#c_call` procedure reuses E0254 ("needs a context, has none"), so **E0258 is still the first
      free code**.
- [x] **The form takes no explicit value**, unlike Jai's `push_context <expr>`, because `Context` is
      unspellable (E0212 — it is the first compiler-declared type and has no `DeclId`). A program has
      no `Context` value to hand it, so it copies the current one.
- [x] **A `defer` inside the block runs against the pushed context** (ADR-0063 §3): the block's own
      defers are emitted before the pointer is restored, so a `defer context.allocator_free(p)` frees
      through the same allocator that allocated. The corpus checks it, and this was the one ordering
      that needed deciding.
- [x] **The formatter dropped the whole statement on the first attempt** — `PUSH_CONTEXT_STMT` was
      absent from `is_stmt_kind`, so the block vanished. `fmt --check` caught it before the corpus
      did. Eleventh wave in thirteen the formatter has lost or damaged source this way.

Two things worth carrying forward.

- **A documented invariant is not a verified one.** ADR-0057 §2's isolation claim survived six waves
  because nothing ran it — no corpus program observed a callee's write *failing* to reach a caller,
  because the shared-context behaviour is what every allocator test wanted. The claim and the code
  disagreed in silence. When an ADR states a runtime property, a corpus program should exercise it.
- **`push_context` is a lowering-time construct**, invisible to sema and resolution — they treat the
  block as an ordinary block. That is why it needed no new type, no `Res` variant and no diagnostic:
  the whole feature is which pointer `context` resolves to in `jr-mir`, and everything above just sees
  a block.
...
- [ ] **Temporary storage is W3's last feature**, and `push_context` was its prerequisite: a scratch
      allocator installed in a `push_context` block is now restored on exit, which is the scoping it
      wanted. What it still lacks is a **bump** allocator, which needs pointer arithmetic (below).
- [ ] **Pointer arithmetic is still refused** (E0223), so a *bump* allocator is not writable — only a
      `malloc` wrapper. The corpus's `recording_alloc` counts bytes but still delegates to `malloc`.
      **Temporary storage wants a real bump allocator, so this is now the blocking gap** — worth an ADR
      of its own (what `p + n` means for each pointee width, and whether it is checked) before the
      temporary-storage wave rather than during.
- [ ] **Traps with backtraces** remains the other W3 feature, and it is independent of everything
      ...
      representation in both engines. A candidate for the next wave, since it is unblocked and pointer
      arithmetic is a decision.
...
- [ ] **`grammar.js` has no gate against loss** — gate 6 checks drift by regenerating, not reversion.
      ...
      enforces; this wave is committed on `feat/push-context`.
