...
Accepted ADRs: 0001–**0220**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
> [!IMPORTANT]
> **ADR-0220 makes switch-arm block placement a real project setting.**
> `[fmt] case_block_style = "same_line"` renders an arm whose entire body is one direct block as
> `case .TEXT; {`; a comment-free empty body is `case .TAG; {}`. `"next_line"` remains the
> formatter default, so existing projects do not churn.
>
> **CLI and LSP share the decision.** The manifest has a compatibility enum separate from the
> formatter enum and `Located::fmt_config()` is the only conversion, so terminal and editor
> formatting cannot assign different meanings to the setting. Unknown values are refused.
>
> **New-project policy is not a global compatibility change.** `jr new` and `jr init` now emit
> `indent_width = 2`; `jr_fmt::Config::default()` remains four spaces for bare files and older
> projects with no manifest.

**1333 workspace tests (1344 under gate 7), 291 corpus files, 220 ADRs, 25 modules, all six required
gates and gate 7 green.** ADR-0220 adds seven tests: three formatter behaviours, three manifest
format guarantees and one CLI project test; the LSP parity test is strengthened in place. No
corpus file moves because this wave changes formatting policy rather than language or engine
behaviour. **E0297** is the first free global diagnostic code;
