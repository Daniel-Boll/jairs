...
That is §5's first named failure mode exactly: a construct the grammar allows, no representation on the
lowering path, filled in with a placeholder that is a **legitimate value**. `Rvalue::Undef` is legitimate
— `c: s64 = ---;` produces one — so neither the verifier nor ADR-0017 §4's poison gate could object, and
nothing read the slot, so nothing failed. The type stored into has no runtime representation *at all*:
`layout_of` answers `LayoutError::ComptimeOnly` for `Item::TypeType`, whose own docs call asking for its
size "a category error". So the compiler was making a slot of something that cannot be stored.

**Only the MIR dump showed it.** `jr check` was silent, `jr run` exited 0, `jr build` exited 0. That is
the third wave running where a false claim survived because nothing *displayed* the contradicting thing —
after ADR-0066's inlining example and ADR-0070's built-versus-optimized MIR. The pattern is specific
enough to act on: when a claim is about a representation, dump the representation.

### What the type-values wave landed

- [x] **ADR-0071 §3**: **E0261**, a type used where a runtime value was expected. Refused in `jr-sema`,
      not in lowering, for ADR-0039 §3a's reason — rejecting a construct is a semantic judgement, and a
      lowering refusal reports a compiler-internal message for a program that looks well-formed.
- [x] **Only two positions had actually escaped**, and knowing which mattered. Every position *with* an
      expectation was already caught by an ordinary mismatch: `takes(Point)` was E0214, `if Point` was
      E0222, `Point()` was E0215, `for x: Point` was E0247. What got through was the two with **no
      expectation to mismatch against** — a `:=` binding and a bare expression statement.
- [x] **The legal positions are an allowlist, not a shape test.** A field-access receiver (`Colour.RED`)
      and a `::` initialiser (`T :: Point;`) are recorded in `type_position` by the code that creates
      each, the same mechanism `call_position` uses for ADR-0059 §5. An allowlist because the failure
      directions are not symmetric: a missed *legal* position is a false error a reader can see and
      report, a missed *illegal* one is the placeholder above.
- [x] **ADR-0071 §2**: a **type alias**. `T :: Point;` used to report "compile-time evaluation failed: a
      file-level item has no value yet" — a const-eval internal on a correct declaration. Its value comes
      from `SigEntry::type_value`, which the signature phase already computed and const-eval is downstream
      of (ADR-0018 §3), so it reads a value that exists rather than inverting a phase. `Item::TypeValue`
      gets its **first producer** since the pool was written, twenty-odd waves ago.
- [x] **Two ADR draft claims were wrong and were corrected before any code was written**, both by
      running. `T : Type : Point;` does **not** parse — the grammar has no annotated-`::` form at all —
...
- **A well-typed placeholder is invisible to every gate this project has.** Three of them now (braceless
  bodies → `Stmt::Error`, an aggregate parameter's field → `Rvalue::Undef`, and a type in a runtime slot).
  What found this one was asking a question no test asks: *what does this lower to?* The corpus asserts
  exit codes and snapshots the built MIR of `valid/`; a construct that is legal, silent and unread by
  anything is outside all of it.
- **Scoping by "does the value exist at run time" split RTTI cleanly.** `Type` values do not exist at run
  time, so they ship with no engine risk; `type_info()` and `Any` do, so they need a runtime type
  representation that `ComptimeOnly` currently forbids. The three were listed together in §2.1 as though
  they were one size of thing. They are not.

Diagnostic codes: **E0262 is the first free code**, **E0131 the first free *parser* code**. This wave
added **one** (E0261). The project defines **92** codes.
...
- [ ] **Then sub-wave 4, `#insert` and the `Code` type**, and §2.1 calls the wave the hardest: full `#run` over arbitrary
