...
waiting. Both are tracked below. **988 workspace tests** and **213 corpus files**, all six gates green, **166
...
> Critical finding. Its remaining findings are the next work, in this order: **no VM fuel** (a `#run while true {}`
> hangs the compiler, and the LSP worker with it), **`BUILD_OUTPUT` is unconstrained** (a program names an
> absolute path and `jr build` writes it), **no cross-crate diagnostic-code uniqueness check** (`jr-hir` and
> `jr-db` have no `code.rs` at all), and **the docs' own drift** — including that `README.md` claims Linux "is
> kept green in CI" when **CI has never run once**.
