...
**W5 — Polymorphism is OPEN, four sub-waves done.** `$T` procedures work end to end: declared, inferred,
instantiated, and run in both engines. **969 workspace tests**, all six gates green, **166 Neovim checks**.
See §1.5.

**What W5 has delivered, in four sub-waves:**
...
- [ ] **Polymorphic structs** — `Box($T) { value: T }` used as `Box(s64)`. **Design of record in ADR-0085**,
      deferred as an implementation because it changes the pool's most load-bearing key: a struct's identity
      is its `DeclId` (ADR-0015 §1), and `Box(s64)`/`Box(bool)` must be distinct types from one decl, so
      `Item::StructType` grows `args: Vec<PoolId>` and the field side-table re-keys from `DeclId` to the
      instance. The stdlib's `Array`/hash table (W7) need this. Start from ADR-0085 §1–§4.
- [ ] **`$$T`** — a comptime-only *value* parameter (`make :: ($N: s64)`), polymorphic over a value known
      at compile time. Interacts with const-eval; its own sub-wave. Does not parse today (`$` on a value
      parameter).
- [ ] **Macros** — `#modify`, `#bake_arguments`, `#expand` (PLAN §2.1). The largest remaining family;
      `#expand` in particular composes with `#insert`/`#code`.
...
Diagnostic codes: **E0269 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
(a polymorphic call that cannot be instantiated — reworded across sub-waves). The project defines **100** codes.
