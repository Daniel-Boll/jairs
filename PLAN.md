...
**W5 — Polymorphism is OPEN, thirteen sub-waves done.** `$T` procedures, polymorphic structs and `$N`
...
Diagnostic codes: **E0276 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
...
predicate began running) and E0275 (an instantiation rejected by its `#modify` predicate — `jr-db`'s). The
project defines **106** codes, one of them retired.

### The next wave

**`#bake_arguments` is the last of W5.** `#expand` and `#modify` are both complete, so what remains of the
macro family is one refinement: `#modify` runs code at compile time to
