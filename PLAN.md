...
> example-bearing top-level chapters in the pinned `The_Way_to_Jai` guide. `jr-cli` turns each row
...
Accepted ADRs: 0001–**0221**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
> [!IMPORTANT]
> **ADR-0221 gives project-facing formatter indentation one default.** Generated projects, empty
> manifests and manifest-free CLI/LSP files all use two spaces. An explicit `[fmt] indent_width`
> still wins, and tabs remain independent of the width. `jr_fmt::Config::default()` stays four for
> direct library callers and the canonical corpus.
>
> **This explicitly amends ADR-0220 §3.** That ADR kept bare files at four spaces while new
> manifests selected two. The decider chose consistency over that compatibility split; the
> historical ADR remains unchanged and this record carries the reversal.
>
> **ADR-0220's case-block setting is otherwise unchanged.**
> `[fmt] case_block_style = "same_line"` still renders a sole direct block as `case .TEXT; {` and
> a comment-free empty body as `case .TAG; {}`.

**1333 workspace tests (1344 under gate 7), 291 corpus files, 221 ADRs and 25 modules.** ADR-0221
adds no test or corpus file; it updates the existing formatter, manifest, CLI and LSP default
assertions. ADR-0220's six required gates and gate 7 were green; for this amendment the decider
explicitly requested focused checks instead of rerunning the full gate set. **E0297** is the first free global diagnostic code;
**E0135** is the first free parser code.

### Next wave: general procedure overloading (roadmap P1)
