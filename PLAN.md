...
**`switch` exists with exhaustiveness checking, and W4.5 is open — a wave earlier than planned.** A
`switch` over an enum must name every member or say `else`; a missing one is E0258 and the message lists
the members rather than counting them. A bare `.RED` works as a case, which settles ADR-0041 §2 step 5.
**924 workspace tests**, all six gates green, plus **162 Neovim checks** verified rather than gated.
See §1.5.

**The wave order itself was the first thing checked, and it moved.** §2.1 placed W4.5 after W4 "because
exhaustiveness diagnostics *want* comptime type info". That is a want, not a need, and it was verifiable:
`Pool::enum_members` already exists and is **populated during checking** (ADR-0041 §4), and both
`c == .GREEN` and `c == Colour.GREEN` already compiled and ran in both engines. So `switch` and
exhaustiveness needed nothing from W4 — what comptime *would* add (exhaustiveness over a computed type,
a generated `switch`) is real but is not what the row asks for. §2.1's row now records the amendment,
because a wave order justified by a dependency that does not exist is exactly §5's "plans that
contradict themselves".

**No new MIR node, and the scrutinee is evaluated once.** A `switch` lowers to the branch chain an
`if`/`else if` over the same comparisons already produces (ADR-0067 §6), so neither back end changed.
The single evaluation is a correctness property, not an optimisation — evaluating per arm would run the
scrutinee's side effects once per comparison — and `valid/054`'s snapshot proves it: one `call proc3` in
`bb0`, with both tests reusing its value.
...
### What the switch wave landed

- [x] **ADR-0067**: `switch` as a statement, cases as *values* compared with `==` (not patterns), `else`
      as the catch-all, no fallthrough. **Three new diagnostic codes** — E0258 non-exhaustive, E0259
      duplicate `case`/second `else`, E0260 unreachable `else` — so **E0261 is now the first free code**,
      the first wave in five to add any.
- [x] **E0260 is what makes E0258 worth having.** Without refusing an `else` on an already-exhaustive
      enum `switch`, every `switch` could end in `else` and the member check would never fire. The three
      diagnostics are one design, not three conveniences.
- [x] **The formatter deleted the whole statement on the first attempt.** `SWITCH_STMT` was absent from
      `is_stmt_kind`, which silently drops a kind, so a formatted `054` lost its four switches entirely.
      ADR-0067's consequences predicted it; formatting the file and reading it is what caught it.
      Twelfth wave in fourteen the formatter has lost or damaged source this way.
- [x] **`describe` and `describe_qualified` produce identical MIR**, which is how the corpus proves that
      `case .RED` and `case Colour.RED` are two spellings of one member rather than two mechanisms.

Two things worth carrying forward.

- **A plan's stated reason is checkable, and worth checking before obeying it.** The W4.5-after-W4 order
  had one justification, and it took three greps and one two-line program to find that it did not hold.
  A wave was gained by reading a rationale sceptically rather than treating the table as given.
- **Exhaustiveness is only honest where the member set is finite.** An `s64` `switch` is not checked,
  because "did you cover every `s64`" has no useful answer — so `else` is what makes a non-enum match
  total. Restricting the check to enums is what keeps the diagnostic true rather than approximate.

Diagnostic codes: **E0261 is the first free code**, **E0131 the first free *parser* code**. This wave
added **three** (E0258, E0259, E0260). The project defines **91** codes.


### Open, and honest about it

- [ ] **W4.5's remaining deliverable is a tagged variant type**, the other half of ADR-0045 §1 — and
      `switch` is what makes it worth having, since ADR-0045 rejected a tag partly because "Jairs has no
      pattern matching" to read one. It needs a representation decision (where the tag lives, how a field
      read checks it) that is independent of `switch` and larger than it, so it is its own wave — and it
      is now unblocked. **This is the next wave**, and it closes W4.5.
- [ ] **W4 — Comptime follows W4.5, and §2.1 calls it the hardest**: full `#run` over arbitrary
      code, aggressive const folding, RTTI (`Type` values, `type_info()`, `Any`), `#insert`, `#code` and
      the `Code` type. Sema and the VM become **mutually recursive**, and the deliverable §2.1 names is
      cycle detection with readable errors — not the features themselves. Estimated 10–14 weeks, which is
      wider than any wave so far and worth breaking into its own sub-waves rather than attempting whole.
- [ ] **`switch` is a statement, not an expression** (ADR-0067 §1), cases are *values* rather than
      patterns (§2) — no destructuring, ranges or guards — there is **no fallthrough** and no multi-value
      `case` (§5), and no jump table (§6): the chain is correct, and a table is an optimisation over the
      same meaning. Exhaustiveness is enum-only (§3).
