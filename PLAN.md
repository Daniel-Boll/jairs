...
| Comptime FFI | **Yes**, gated behind `#foreign_at_comptime` | VM needs libffi-style dynamic calls. ~~Non-negotiable given build scripts must read files.~~ **That reason was wrong, and ADR-0195 proved it by shipping the feature it was justifying**: a build script runs as an ordinary *program*, so it reads files through the stdlib and needs no comptime FFI at all. Still wanted for its own sake — a `#run` that reads a schema and generates code — and no longer blocking anything. |
...
- [x] CI green on macOS arm64 **and** Linux x86-64 — all seven jobs pass. ADR-0205
      and ADR-0206 record the eight failures the first observed Linux runs exposed,
      including a silent System V aggregate miscompile and a host-specific MIR snapshot.
      The `corpus-drift` job now runs the real compiler/tree-sitter checks, validates both
      editors' queries, and fails on generated-parser/query drift (ADR-0222).
...
>
> **Current project catalog (ADR-0213).** `jr-project` discovers manifest-backed direct `src`
> modules, exact named path dependencies, bundled modules, and compatibility roots once, outside
...
>
> **Current public String surface (ADR-0225).** The pinned Way-to-Jai guide now has its evidenced
> non-overload spellings: `slice` is a strict borrowed view, `copy_string` is an owned copy, and
> `begins_with`, `compare_strings`, the two substring-index names, `replace_chars` and `is_any`
> delegate to the existing algorithms. `string_to_int` and `string_to_float` expose the guide's
> two-result conversions; the richer float prefix parser is `to_float`, and `parse_int(*string)`
> advances the caller's view on success. Ownership comments distinguish borrowed, owned and
> transferred storage. Byte/string overload families remain queued for general overloading.
>
...
>
> **Current exhaustiveness action (ADR-0218).** E0258 now offers one preferred `add all missing
> cases` quick fix for enums and tagged variants, in both `switch` and `if #complete`. The action
> consumes sema's declaration-ordered runtime-value classes, inserts explicit empty bare-member
...
> **Current parameterised inference surface (ADR-0229).** A procedure parameter such as
...
> **ADR-0240 component delta.**
>
> | Component | Current delta |
> |---|---|
> | `jr-lsp` | Field completion now handles `Item::DynamicArrayType`: `[..]T.` offers `data: *T`, `count: s64`, and `capacity: s64`, including `data: **Node` for `[..]*Node`. The incomplete query renders the pointer detail from the known element type without interning a new pool item. |
...
> **ADR-0246 component delta.**
>
> | Component | Current delta |
> |---|---|
> | `jr-refactor` | One protocol-neutral extraction seam correlates exact byte selections with resolved HIR. Safe expression schedules become locals; consecutive statements become nested procedures with pointer-preserving captures, caller-owned escaping locals/return slots, outward-flow dispatch, writable type spelling, deterministic names, and explicit refusals. |
> | `jr-lsp` | `refactor.extract` is advertised with immediate edits, strict non-clamping ranges, project indentation, and `context.only` filtering. Semantic tokens append a stable boolean wire kind and classify explicit struct-literal type operands before ordinary expression resolution. |
> | Hover / editors | Cards keep the container outside a signature-only `jairs` fence. A narrow Tree-sitter recovery capture highlights bodyless hover procedure signatures in Neovim and Zed; boolean query captures remain values. |
> | Tests / boundary | Five interface tests apply and type-check extraction edits or pin an honest defer refusal, two LSP tests exercise both actions and filtering, and focused token/range/render tests pin wire indices, struct-literal context, stale ranges, and markdown shape. Unsupported evaluation schedules, generated source, unspellable types, and changed defer lifetimes remain honest non-offers. |
...
|---|---|---|
| `jr-refactor` — ADR-0246 delta | **Done, deliberately conservative** | Protocol-neutral extraction owns semantic selection, capture/storage preservation, escaping declarations, outward-flow translation, writable type spelling, hygienic names, and exact source edits. Unsupported schedules and lifetimes are refusals rather than approximate rewrites. |
...
| `jr-project` | **Done** | **ADR-0213 replaces query-time path probing with one project-owned catalog.** Manifest projects implicitly expose direct `src/Foo.jr` and `src/Foo/module.jr`; `[dependencies]` maps names to exact files or directory `module.jr`; bundled modules and compatibility `-I`/`[build].module_paths` roots enter the same sorted catalog. Discovery and reads happen before salsa. Local/exact duplicates fail, implicit/legacy modules cannot shadow bundled names, and exact dependencies or operator `-I` can. |
...
| `jr-fmt` | **Works, safety redesign pending** | **ADR-0222 closes ADR-0220/0221's split default:** generated projects, direct library callers, empty manifests, manifest-free files, examples, modules and the canonical corpus all use two spaces unless `[fmt] indent_width` says otherwise. **ADR-0220 adds one shared configuration seam for switch-arm blocks:** `[fmt] case_block_style = "same_line"` prints an arm whose sole statement is a block as `case .TEXT; {`, with a comment-free empty body as `{}`; `next_line` remains the compatibility default. CLI and LSP consume the same manifest conversion. **ADR-0212 fixes a formatter-induced ABI break:** `#program_export` was silently deleted, so a library procedure kept compiling under a private mangled symbol and its C caller could no longer link it. The recurring structural cause remains: raw `SyntaxKind` allowlists and wildcard fallbacks duplicate typed AST roles, so a typed exhaustive dispatcher is still the recommended safety redesign. |
...
| `jr-lsp` | **Done, not finished** | **ADR-0246 adds semantic extraction and editor fidelity:** strict selection ranges feed protocol-neutral local/procedure extraction, `context.only` is honoured, booleans use an appended value token, explicit struct-literal operands are types, and hover signatures live alone in a `jairs` fence. **ADR-0240 restores dynamic-array field completion:** `stack: [..]*Node; stack.` offers `data: **Node`, `count: s64`, and `capacity: s64`, matching sema without mutating the pool from an incomplete editor query. **ADR-0218 adds the missing exhaustiveness assist:** E0258 offers explicit empty arms for every uncovered enum runtime-value class or tagged-variant case, for both `switch` spellings. It reads sema's list rather than duplicating member analysis, preserves configured indentation and CRLF, converts through the negotiated UTF-8/UTF-16 position map, and refuses a stale client diagnostic. **ADR-0211 inventories the advertised surface:** compiler diagnostics; hover and definition including type positions; completion + resolve and auto-import; references and document highlights; document/workspace symbols; prepare-rename + workspace rename; code actions; signature help; inlay hints; full-document semantic tokens; whole-document formatting. Rename is workspace-wide and refuses rather than half-renaming. The remaining correctness work includes stale importer diagnostics, lossy secondary ranges and instantiation frames, stale/scopeless completion, and protocol negotiation/lifecycle hardening. |
...
Accepted ADRs: 0001–**0246**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
| **W12 — Debug info** ✔ **MOSTLY DONE — ADR-0169–0174, ADR-0200** | DWARF line tables, named type/struct-layout DIEs, subprograms and stack-resident locals in both native back ends | Cranelift and LLVM now emit real source lines and types, and imported nominal types carry their source names. The remaining item is register-resident locals: Cranelift value labels cover ranges, so correctness needs `.debug_loclists`, not one whole-function register location. `dsymutil` packaging remains a distribution concern rather than missing debug information. | not estimated |
...
**1447 workspace tests (1460 under gate 7), 328 corpus files outside fixture modules, 246 ADRs and
...
ADR-0246 makes editor refactoring semantic rather than textual. A new `jr-refactor` crate returns
exact byte edits or explicit refusals; the LSP adapts them to strict negotiated positions and
advertises `refactor.extract`. Procedure extraction preserves capture storage by pointer, hoists
locals used afterward, and dispatches outward `return`/`break`/`continue` in the caller. Semantic
tokens now distinguish boolean values and explicit struct-literal type operands, while hover emits
a signature-only `jairs` fence consumed by both editor query sets. No MIR node, layout
representation, pool layout, or back end changed, so gate 7 is not required.
...
1. canonical, context- and scope-aware completion with negotiated snippets;
2. LSP lifecycle, capability negotiation, protocol errors and cancellation;
3. formatter exhaustiveness and a real `fmt --check` unified diff (configuration parity is closed);
4. executable dead-section elimination — measured at **134,056 → 111,728 bytes** for hello and
   **156,600 → 131,856 bytes** for Snake, with link time effectively unchanged;
5. type-definition, implementation/call hierarchy, parameter hints and richer assists;
6. measure and retune MIR inlining, whose current `-O1` growth has no demonstrated benefit on the
...
six times and **has had its own ADR since ADR-0151** — this sentence said it was still owed until ADR-0205 checked, and `docs/adr/0151-must-and-the-error-model.md` is titled "the error-handling marker ADR-0008 chose and never built". Today a failure is either a trap (which ends the program) or
