...
| `jr-fmt` | **Works, safety redesign pending** | **ADR-0212 fixes a formatter-induced ABI break:** `#program_export` was silently deleted, so a library procedure kept compiling under a private mangled symbol and its C caller could no longer link it. The regression asserts both legal orders beside `#c_call`, because reparsing alone stayed green on the broken output. The optimisation audit also found the recurring structural cause: raw `SyntaxKind` allowlists and wildcard fallbacks duplicate the typed AST's node roles. A typed exhaustive dispatcher remains the recommended next formatter wave; one repaired arm is not presented as an architectural fix. |
...
Accepted ADRs: 0001–**0219**. See [`docs/adr/README.md`](docs/adr/README.md). The repeated stale
...
> [!IMPORTANT]
> **ADR-0219 establishes the executable compatibility baseline.** One strict TOML manifest names
> exactly one representative from every example-bearing top-level chapter in the pinned
> `The_Way_to_Jai` guide: 03–31 except 32, plus 33–35 and 50–52. Thirty-five repository-owned
> probes now execute as named tests through the real `jr` command boundary.
>
> **The submodule remains provenance, not an input.** Every probe and sibling fixture is copied
> into a fresh temporary directory before `check`, `run`, `build`, or native build-and-run. The
> runner never reads `references/The_Way_to_Jai`, so an ordinary clone with no initialized
> submodule has the complete gate.
>
> **The baseline says exactly what it proves.** Six selected examples are source-compatible at the
> pinned revision; the other rows are runnable ports, exact diagnostic blockers, or intentional
> divergences. Thirty-five representatives make chapter coverage executable, not all 315 examples
> supported, so no compatibility percentage is published.

**1326 workspace tests (1337 under gate 7), 291 corpus files, 219 ADRs, 25 modules, all six required
gates and gate 7 green.** ADR-0219 adds 36 dynamic integration tests — one manifest invariant and
35 probes — and no corpus file, because compatibility evidence is deliberately not the language
specification or engine differential input. **E0297** is the first free global diagnostic code;
...
3. formatter exhaustiveness, CLI/LSP configuration parity and a real `fmt --check` unified diff;
