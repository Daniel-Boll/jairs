...
**`variant` exists, and wave W4.5 — Pattern matching is complete.** A `variant { … }` is a tagged union:
a write sets the tag, a read of a different case **traps** rather than reinterpreting bits, and `switch`
destructures it by case with the same exhaustiveness check an enum gets. That is the third and last of
§2.1's W4.5 deliverables, after `switch` and exhaustiveness (ADR-0067). **928 workspace tests**, all six
gates green, plus **166 Neovim checks** verified rather than gated. See §1.5.

**This followed ADR-0045 §1's instruction rather than reversing it.** That ADR rejected a tagged `union`
on three grounds and said what to do instead: "a *tagged* variant type becomes worth having — and it
should be a **different declaration form** then, the way `enum_flags` is different from `enum`". Two of
its three grounds had since gone away (Jairs has pattern matching now, and therefore a way to ask which
case is live); the third stands — a variant is bigger than the equivalent union — which is precisely why
it is a separate form. `union` is untouched, still untagged, still one word smaller, and a corpus program
plus a differential test prove it still reinterprets.

**The wave's two real bugs were both in passes, and both found by running.** DCE deleted the tag store
*and* the value store, because a `TagCheck` was grouped with the writes rather than counted as an
observation — a correct read then trapped with `tag=0`. The same grouping mistake in the slot-liveness
collector panicked with "a live place named a dropped slot". Neither is a lowering placeholder, but both
are the same failure mode §5 names, reached through an omitted classification in a pass: the
exhaustive-match discipline forced me to *touch* every site and I still put two of them in the wrong
group.
...
### What the variant wave landed

- [x] **ADR-0068**: `variant` as a third aggregate form, a leading `u8` tag, a checked read, and
      `switch` over its cases. **No new diagnostic code** — exhaustiveness reuses E0258/E0260, which is
      the evidence that reusing ADR-0067's machinery was the right shape rather than a coincidence.
      **E0261 is still the first free code.**
- [x] **The one line that makes a variant a variant**: every case sits at the payload offset, not at 0.
      Pinned by two `jr-pool` tests, the second being the case an 8-aligned-only test would hide — two
      `u8` cases give size 2 with the cases at offset 1.
- [x] **The formatter destroyed the declaration.** `format_struct_type`'s two-way `if` had an `else`
      branch meaning "struct", so every `variant` became a `struct` — and that function's own docs
      already warned about this exact mistake for `enum_flags`. A match on the kind now, and the
      thirteenth wave in fifteen that the formatter has lost or damaged source.
- [x] **ADR-0068 §5 was corrected mid-wave.** Its draft wrote `case i;`, which does not work: a bare `i`
      goes through ordinary name resolution and reports E0201. The spelling is `case .i`, reusing
      ADR-0046's "the context supplies the namespace" — a smaller change than a new resolution rule, and
      it makes a variant arm read exactly like an enum's `case .RED`.

Two things worth carrying forward.

- **An ADR's own examples are worth running while writing it.** Two waves in a row shipped a draft whose
  illustration was wrong — ADR-0066's inlining example and ADR-0068's case spelling — and both were
  caught in minutes by typing the program. The forks were sound both times; the prose was not.
- **A new variant in a shared enum is only half the work.** `AggregateKind` and `TagCheck` each made
  every match site a compile error, which found the sites — but a compile error only asks "which group?",
  and answering it wrongly is silent. The two bugs above were both wrong answers to that question.
