...
**W4 sub-wave 6 is COMPLETE (ADR-0074): an aggregate compile-time value.** `V :: #run mk();` where `mk`
returns a struct or an array now works — the value is interned and both engines materialise it. This is what
ADR-0073 §0 found `type_info()` and `Any` were *really* blocked on, so after it RTTI is a schema decision
rather than a representation one. **958 workspace tests**, all six gates green, **166 Neovim checks**.
See §1.5.

**The design is one sentence:** an aggregate constant interns as its **element values**, not as the byte
image the VM already had — because `layout_of` takes a `TargetLayout` and the pool holds none, so bytes
would put one target's padding into a shared table. Each engine converts values to bytes itself, at the
point that knows which target is meant. `valid/061` reads a struct, an array, a nested aggregate and a
local copy, and exits **45** in both engines.

### What the wave landed

- [x] **`Item::AggregateValue { ty, elements }`** — the pool's first *recursive* value variant, so every
      exhaustive walk had to decide what a nested aggregate means. That is how all fourteen sites were
      found rather than remembered, and a nested aggregate then needed no special case: an element is an
      interned value, so the representation recurses by construction.
- [x] **The `ty` field, which the ADR draft omitted and the compiler caught in minutes.** `type_of` is
      total, and two struct types with identically-typed fields produce the same element list — an
      elements-only key would intern them to **one id**, so a constant of one type would silently stand in
      for the other. The same reason `IntValue` and `ProcValue` carry one.
- [x] **Two materialisations from one shared value** (ADR-0019's arrangement): the VM writes each element
      at `field_offset`, and the native back end materialises a stack slot exactly as it does for a
      string's `{data, count}` pair. The differential asserts the **value** 45, not merely agreement — a
      layout disagreement would give both engines a consistent wrong number.
- [x] **`Res::Item` gained a place for an aggregate constant**, by spilling it into a slot. Without it,
      reading `V.x` refused the body with "a memory reference has no place": a projection needs an address
      and a constant is an operand. A *scalar* constant still has none and must not — a slot for every
      `LIMIT :: 4096;` would be memory for nothing.
- [x] **A union constant is refused** (§4): untagged storage makes "which field is valid" unanswerable, and
      picking one silently is the reinterpretation ADR-0045 §1 allows only for a runtime read the
      programmer wrote. Tested by CLI exit code rather than a corpus file, because **E0230 is `jr-db`'s
      code and no corpus directory holds one** — `type-errors/` is `jr-sema`'s and `cfg-errors/` is
      `jr-mir`'s, so filing it in either would break that directory's stage contract.
- [x] **The LSP renders an aggregate on hover** (`{7, 0}`), which its own doc comment had said was not
      worth it. That was true when the only possible representation was a byte image and false once the
      elements are right there — a comment made stale by a decision taken elsewhere.
- [x] **Teeth-checked twice**, because two separate mechanisms are load-bearing: disabling the interning
      and disabling the place-spill each make `valid/061` fail rather than silently miscompile.

One thing worth carrying forward.

- **Reading a field of an aggregate constant spills the whole constant, once per read.** `POINT.x + POINT.y`
  materialises `POINT` twice, which the corpus snapshot shows plainly. Correct but wasteful, and recorded in
  ADR-0074's consequences rather than left to be discovered: caching the slot per `(item, body)` is the
  obvious fix, deliberately not in this wave because a cache keyed on the wrong thing is a wrong *address*
  rather than a slow program.

Diagnostic codes: **E0265 is the first free code**, **E0131 the first free *parser* code**. This wave added
**none** — it lifted a refusal rather than adding one, and reworded E0230's message, which used to call
every aggregate "a struct value". The project defines **95** codes.
...
- [ ] **`type_info()` and `Any`** — **now unblocked** (ADR-0074). The representation they waited on exists:
      an aggregate constant interns, so a describing struct the compiler manufactures has somewhere to live.
      What remains is a *schema* decision rather than a representation one — what fields a `Type_Info`
      struct has, declared in `modules/Basic` and populated by the compiler — plus, for `Any`, a
      `{type, pointer}` pair and rules for what may go in one and how it is read back out. `Any` is the
      larger of the two and depends on the first, so `type_info()` goes first.
      **One thing to check before designing the schema**, because it is the kind of claim this project has
      found false four times: whether a *type* can be a field of that struct at all. `Item::TypeType` is
      `ComptimeOnly` (ADR-0071), so `Type_Info.element_type` may need to be an opaque handle rather than a
      `Type` — probe it with a two-line program before writing §1.
