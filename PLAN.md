...
**W5 — Polymorphism is OPEN, six sub-waves done.** `$T` procedures, polymorphic structs, and `$N`
comptime-value parameters (both the surface *and* instantiation) work. **977 workspace tests**, all six
gates green, **166 Neovim checks**. See §1.5.
...
- [ ] **`[N]T` over a `$N` parameter** — the case that meets polymorphic structs (`buf: [N]T`).
      `constant_array_length` (ADR-0070 §1) currently reads only a *file-level constant*'s literal; a `$N`
      parameter's baked value is not yet a source it consults. The plumbing is trivial once the value is
      wired through, and it lifts the *most useful* case for `$N`. **Recommended next mini-step.**
...
because constancy is a const-eval judgement). The project defines **103** codes.

### The next wave

**`[N]T` over `$N` next (a mini-step), then macros.** `$N` instantiation is done and both engines agree on
its behaviour (`valid/072` exits 32); the one useful piece still owed is `[N]T` where `N` is a `$N`
parameter, which needs `constant_array_length` to consult the baked value the way it already consults a
file-level constant's (ADR-0070 §1). Small, and it lifts the case that meets polymorphic structs (`buf:
[N]T`). Before starting, confirm the premise — that `constant_array_length` is genuinely the one path an
array length resolves through, and that a baked value lives somewhere it can read (the instantiation's
`ProcId` and the parameter index). Macros are the larger family after that; **the `Code` value ADR-0080
declined is finally revisited when `#expand` reads a macro's argument**, because that is what supersedes
"splice-only" with a real representation.
