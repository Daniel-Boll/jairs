...
**W4 sub-wave 5 is COMPLETE (ADR-0073): a computed `#insert` operand works end to end.** `#insert CODE;`
and `#insert #run build();` evaluate the operand's string at compile time and splice its text into the
enclosing scope. This is the point PLAN §5 named the project's top risk — sema and the VM mutually
recursive — and the cycle is broken by an acyclic pre-pass, not salsa's fixed-point recovery. **954
workspace tests**, all six gates green, **166 Neovim checks**. See §1.5.

**The model:** `#insert <expr>;` is `#insert #run <expr>` in spirit — evaluate the operand to a string,
then substitute exactly as the literal form (ADR-0072). `valid/060` sums a named-constant operand, a
`#run` operand, an empty operand and a nested computed one to exit **58**.

### What the wave landed

- [x] **The parser accepts `#insert <expr>;`** — the literal-string CST is byte-for-byte unchanged, so
      ADR-0072's lowering is untouched, and the tree-sitter grammar needed no change (it parses directives
      generically). An ADR draft claim that gate 6 gained work was corrected by running `tree-sitter parse`.
- [x] **`Stmt::Insert { operand: Option<ExprId> }`.** A computed operand is lowered as an ordinary
      expression and resolves and type-checks: `#insert undefined;` is **E0201**, a non-`string` operand is
      **E0214** at the operand's span — real diagnostics, not a blanket E0262.
- [x] **`insert_operands` query + inline expansion in `file_mir`.** The query reuses `file_consts`'
      evaluator (one evaluator, one cycle detector) and keys results by directive **span** — invariant
      across the re-lowering, unlike the operand `ExprId`. Expansion runs inline in `file_mir`
      (`lower_file_with_inserts` → `checked_expanded` re-resolves and re-checks the expanded tree → lower),
      needing **no new salsa queries** because `resolve` and `check_file` take an explicit `&FileHir`. A
      file with no computed insert takes the ordinary path untouched.
- [x] **Acyclic, verified:** `frontend_diagnostics` is mir-free (only `file_diagnostics` reaches
      `file_mir`), so `insert_operands → file_consts → frontend_diagnostics` never loops — as ADR-0073 §1
      claimed. Never rewired `imports_of`/`file_exports`/`file_signatures` (item-only; `#insert` is
      body-scoped), which is what keeps the import cycle broken.
- [x] **Withheld E0201 in a body holding a pending insert** (it cannot know what the insert declares), with
      the *expanded* resolve reporting real errors via `MirResult::expanded_diagnostics` → `file_diagnostics`.
      Without it a typo in such a body read as "the compiler could not lower `main`".
- [x] **A depth bound, E0264** (§3), refused past 16 levels with a diagnostic — the guard a literal insert
      did not need, because a *generated* string can reproduce itself without growing. Teeth-checked.
- [x] **An expanded insert clears `operand` to `None`.** An operand evaluating to `""` expands to zero
      statements, and keeping `operand: Some` would make it the *pending* state `scan` refuses. Clearing it
      separates "evaluated-empty" from "not yet evaluated" — found by probing the empty case.
- [x] **`jr-fmt` no longer drops a computed operand.** `#insert CODE;` had formatted to `#insert;` — the
      CST-preservation failure ADR-0072 §1 warned of, caught by the corpus `jr fmt` gate. `DIRECTIVE_EXPR`
      now formats an operand expression, not only a bare string token.
- [x] **Corpus:** `valid/060` (runs, exit 58, asserted as a *value* in the differential per
      [[jairs-assert-behaviour-not-agreement]]) and `type-errors/064` (non-string operand is E0214).
- [x] **The safety net held at every intermediate commit:** a computed insert was diagnosed, never
      miscompiled — teeth-checked by disabling expansion, which makes `scan` refuse the body.

Two things worth carrying forward.

- **The corpus differential checks agreement, not correctness.** Every construct with an observable result
  owes one assertion of the *value* — `060`'s exit 58 and `059`'s exit 64 are those, and both would pass a
  differential that agreed on a wrong answer. [[jairs-assert-behaviour-not-agreement]].
- **A two-pass lowering must key cross-pass results by `Span`, not by an arena id**, which shifts when an
  earlier insert expands. [[jairs-insert-operand-key-by-span]].

Diagnostic codes: **E0265 is the first free code**, **E0131 the first free *parser* code**. This wave added
**three** (E0262/E0263 in sub-wave 4, E0264 here) across the two `#insert` sub-waves. The project defines
**95** codes.
...
- [ ] **`type_info()` and `Any`** remain W4's other open branch, and ADR-0073 §0 showed they are a
      *different* problem from this cycle: both make a type into **runtime data**, blocked on `jr-pool`
      having **no aggregate value variant at all** (a `#run` returning a struct is E0230, "a compile-time
      struct value arrives with a later wave"). The thing to lift first is that E0230 — an
      aggregate-constant representation — after which `type_info()` is a describing struct in
      `modules/Basic`, and `Any` is that plus a `{type, pointer}` pair.
