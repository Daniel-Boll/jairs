...
| **W4 — Comptime** | Full `#run` (arbitrary code), aggressive const folding, RTTI (`Type` values, `type_info()`, `Any`), `#insert`, `#code`, the `Code` type | **Hardest wave.** Sema ↔ VM become mutually recursive; cycle detection with readable errors is the deliverable. **Delivered in sub-waves** (ADR-0069 §0), because a wave five times the size of any other cannot be verified the way the others were: (1) `#run` across files and in a body — *done*; (2) an array length from a constant — *done*, and it replaced "aggressive const folding", which ADR-0070 §0 found ADR-0022's const-prop had already delivered; (3) RTTI — **`Type` values done** (ADR-0071), `type_info()` and `Any` outstanding, split on whether the value exists at run time; (4) `#insert` and `Code` | 10–14 wks |
...
**W4 sub-wave 7 is COMPLETE (ADR-0075): `type_info(T)`, and a constant that may hold a string.**
`type_info(Point)` returns a `Type_Info` giving a type's kind, name, size and alignment. This is the first
half of RTTI — `Any` is still owed. **960 workspace tests**, all six gates green, **166 Neovim checks**.
See §1.5.

**It took two decisions, and the first was not supposed to be needed.** ADR-0074 closed by calling itself
"the last thing between W4 and RTTI". Typing one program showed it was not: a compile-time aggregate
holding a **string** was still refused, and a `Type_Info` carrying a type's *name* is exactly that shape.
That is the **fourth** false scheduled dependency (ADR-0067 §0, ADR-0070 §0, ADR-0072 §5, ADR-0073 §0) and
the first where the false claim was this project's own ADR about the very next wave.

### What the wave landed

- [x] **`Raw::Aggregate` became a tree** (§1). The refusal was structural, not a missing branch: `reduce`
      copied a result out as a flat byte image, and a `string` field's bytes are a `{data, count}` pair
      pointing **into the VM's memory**, which is gone by the time interning runs. Elements are now reduced
      *while the VM is alive*, so a string reaches `read_string` at the one moment it can still be read.
      Not a new mechanism — `reduce` already did this for a *top-level* string; the decision was to apply
      it one level down. `aggregate_placements` is now the single answer to "which shapes have readable
      elements and where", shared by the walk and by interning, because two copies would be two chances to
      disagree about an offset.
- [x] **`Type_Info` is declared in `modules/Basic`, in Jairs** (§2), because it must be *spellable*: a
      program that reflects has to write `info: Type_Info`. Probing found **no** compiler-declared type is
      spellable — `t: Type;` and `c: Context;` both report E0212, since `Context`'s fields are a Rust
      `const` and such a type has no `DeclId` (ADR-0057 §1). Declaring it in Jairs makes it an ordinary
      nominal struct, so field access, layout, copies and pointers need no new case.
- [x] **The dependency on `Basic` is validated, not trusted.** `TYPE_INFO_FIELDS` is the contract, and a
      mismatch in field names, types or order is **E0265** naming it — not a read of whatever now sits at
      the old offset. A wrong offset is a silent wrong value, which is the failure mode ADR-0017 §4 says
      must refuse instead.
- [x] **Returns a `Type_Info` by value, and ADR-0075 §2 first said pointer.** The MIR **verifier** caught
      it minutes after the first working build — `deref of a non-pointer` — because the folded value is an
      `Item::AggregateValue`, a constant, which has no address. A pointer needs the pointee somewhere: a
      stack slot dangles when the frame returns, static data per type is a storage decision this wave
      declined to also make. By value needs neither, since an aggregate return is ADR-0051's `sret`. And §4
      had never promised two calls share a pointer, so nothing was given up.
- [x] **`type_info(s64)` works**, which took two *withheld* refusals: the builtin names are ordinary
      identifiers rather than keywords, so `s64` resolves to nothing and E0201 fired. The withholding is
      scoped to a `type_info` argument, so `x := s64;` elsewhere keeps its error — ADR-0071 §3's asymmetry
      argument, one level down.
- [x] **The refusals say what they mean.** `type_info(x)` for a local first said "unknown type name `x`" —
      wrong twice over, since `x` is well known and the objection is what it *is*. The builtin lookup is now
      matched by text with **no diagnostic** and only for a genuinely unresolved name, so the case is E0261
      (`type-errors/065`).
- [x] **Two more things found by running.** `file_consts` returned early when a file had no `wanted` target,
      leaving a `type_info`-only file's call unfolded and refused as "a name failed to resolve". And E0265
      fired in `jr-sema`'s own corpus test, which checks *without* module resolution deliberately — that was
      inventing a library error out of a missing input, so the lookup is silent when no imported signatures
      were supplied at all.
- [x] **`jr-mir`'s dump stopped printing an imported enum's `DeclId`.** `Type_Info_Kind` lives in `Basic`,
      so it fell through to a fallback rendering a `FileId` — assigned in database load order, so one new
      corpus file renumbered every occurrence. Exactly the snapshot churn `AGENTS.md` forbids, and the
      reason `extern proc3` exists.
- [x] **Teeth-checked twice**, because two mechanisms are load-bearing: disabling the intrinsic recognition
      makes `valid/063` fail, and dropping the recorded constant makes the program **refuse** rather than
      silently lower a placeholder.

Diagnostic codes: **E0267 is the first free code**, **E0131 the first free *parser* code**. This wave added
**two** — E0265 (`Type_Info` missing or wrongly shaped) and E0266 (`type_info` of a type with no runtime
layout). The project defines **97** codes.

### Open, and honest about it

- [ ] **`Any`** — the second half of RTTI, and it needs `type_info` first, which now exists. A
      `{type, pointer}` pair, plus rules for what may go in one and how it is read back. The open question
      is whether *every* value coerces to `Any` implicitly, which is a conversion decision rather than a
      representation one.
- [ ] **Per-kind detail in `Type_Info`** (ADR-0075 §3) — a struct's field list, an array's element type, a
      procedure's signature. Each is a **variable-length** member, so it wants a view or a
      pointer-and-count, and that raises a memory-ownership question: static data the back end emits, versus
      a comptime-built table. Deliberately deferred rather than half-done; the schema extends by adding
      fields, which does not break a reader that names only the four it has.
      **One thing already checked**, so the next designer need not: a `Type_Info` cannot hold a `Type`
      field, because `Item::TypeType` is `ComptimeOnly` and has no runtime layout. An element type has to be
      an opaque handle or a nested `Type_Info`, not a `Type`.
- [ ] **`#code` and the `Code` type** (ADR-0072 §4, ADR-0073 §4) are the last of the `#insert` family: a
      quoted syntax tree as a first-class value. The first question is ADR-0071 §4's — does it exist at run
      time? Likely not, so it is comptime-only like `Item::TypeType` and wants the treatment `Type` got. It
      follows `#insert` because it is only useful once something can splice it, and now something can.
- [ ] **A `#run` reading another file's constant** stays refused, and ADR-0073 §4 corrected the plan: it
      does **not** come free with this wave, because the narrow acyclic pre-pass is precisely what does not
      deliver the general cross-file value dependency. It needs its own decision.
- [ ] **`#insert` at file scope** stays refused (ADR-0072 §5): it would change the item tree, so the
      signature phase would see declarations no `#import` and no file walk produced.
