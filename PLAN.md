...
Accepted ADRs: 0001–**0189**. See [`docs/adr/README.md`](docs/adr/README.md). (This line
...
Tests **hold at 1082**; the workspace corpus **269 → 270** `.jr` files under `tests/corpus/` outside
`tests/corpus/modules/`. ADRs 188 → **189**. **No new diagnostic code** — E0295 is still the first free
...
**Newly owed, from ADR-0189, ranked by reach:**

1. **A `*Type_Info` per type**, using the compiler-emitted static-data table ADR-0152 §3 already built for
   `fields`. This is **one change that lifts four visible gaps together**: an enum prints its ordinal
   rather than its member name, a nested aggregate field prints `…`, a view prints `<view>`, and a
   structural type's `name` is the lowercased kind (`array`, not `[3]s64`). All four have the same root —
   `Type_Info_Field.ty` and `Type_Info.element` are type **ids**, and ADR-0077 §1 makes an id deliberately
   opaque, so nothing can recurse. `format_field` works around it with thirteen constant comparisons,
   which covers scalars and stops there. The highest-leverage library-facing item this project has.
2. **`any_as` on a pointer type.** `any_as(a, *Point)` is E0261 — the second argument must be a named
   type — so an `Any` holding a pointer (which ADR-0189 §2 made the *common* case) cannot be read back.
   Found by migrating a test, and worked around there rather than built.
3. **A field width or precision for `%`.** `print` renders shortest-ish and takes no modifiers, so a
