...
**W5 — Polymorphism is OPEN, five sub-waves done.** `$T` procedures *and* polymorphic structs work.
**974 workspace tests**, all six gates green, **166 Neovim checks**. See §1.5.
...
- [ ] **`$$T`** — a comptime-only *value* parameter (`make :: ($N: s64)`), polymorphic over a value known
      at compile time. **The recommended next sub-wave** — smaller than macros and it reuses the
      instantiation machinery, keying on the *value* rather than the type. It interacts with const-eval (a
      `$N` argument must be a compile-time constant, evaluated at the call — the sema↔VM recursion W4
      already broke once) and does not parse today (`$` on a *value* parameter). Its most useful case is a
      length: `buf: [N]T` inside a `struct($T)`/`($N: s64)`, which is where it meets polymorphic structs.
...
Diagnostic codes: **E0271 is the first free code**, **E0132 the first free *parser* code**. W5 added E0268
(a polymorphic call that cannot be instantiated — reworded across sub-waves) and, for polymorphic structs,
E0269 (a `Name(args)` that is not a parameterised struct, or is cross-file) and E0270 (a wrong type-argument
count). The project defines **102** codes.

### The next wave

**`$$T` next, then macros.** Before starting `$$T`, check its stated premise against what exists — that
habit has now caught a false schedule *five* times (ADR-0067 §0, ADR-0070 §0, ADR-0072 §5, ADR-0073 §0, and
ADR-0075's own closing claim, which ADR-0078 had to correct). In particular: `$$T` is claimed not to parse
today, so confirm the parser genuinely rejects `$` on a value parameter before designing its lowering, and
confirm the const-eval-at-a-call-site path `$N` needs is the same `insert_operands`/`file_consts` machinery
W4 built rather than a second evaluator. `$$T` is recommended over macros because it is smaller and reuses
the instantiation harness; macros are the larger family and want `#expand` composing with `#insert`/`#code`.
