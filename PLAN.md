...
**W4 sub-wave 9 is COMPLETE (ADR-0078): `Type_Info`'s fixed-size per-kind facts.** `Type_Info` gained
`count` (a struct's field count or an array's length) and `element` (an array's element or a pointer's
pointee, as a type id). **963 workspace tests**, all six gates green, **166 Neovim checks**. See §1.5.

**This split ADR-0075 §3's deferred "per-kind detail" into its two halves.** The variable-length members —
a struct's field *list*, a procedure's signature — still need somewhere for their elements to live (static
data vs a comptime table) and stay deferred. The fixed-size facts do not: a count is a number, an element
type is a pool id (an `s64` since ADR-0077), so they ship with no new representation, no query, no engine
change. `element` is an id rather than a `*Type_Info`, because a pointer would need the element's info
built and stored — the deferred decision.

### What the wave landed

- [x] **`count` and `element` on `Type_Info`**, filled for struct/union/variant (field count), array
      (length + element id) and pointer (pointee id); 0 for every other kind, which is a real answer.
      `TYPE_INFO_FIELDS` **appends** them, so the earlier fields' offsets are undisturbed — the third
      exercise of "the schema extends by adding fields" after ADR-0077's `id`.
- [x] **A procedure's parameter count is deliberately not set** (ADR-0078 §3): that is the
      variable-length list, and summarising only its length would imply the elements are reachable.
- [x] **The dump's `Type_Info` shape detector matches seven fields**; `#id` masking unchanged.
- [x] **Array/pointer facts are implemented but not reachable from source**, because `type_info([4]s64)`
      does not parse and a structural alias is ADR-0071 §5's deferred fixpoint — recorded in ADR-0078 §4,
      not worked around.

Diagnostic codes: **E0268 is the first free code**, **E0131 the first free *parser* code**. This wave
added **none**. The project defines **98** codes.
...
- [ ] **A structural type argument** — `type_info([4]s64)`, `type_info(*T)`. The argument grammar is
      expression-only, and a structural type alias (`Arr :: [4]s64;`) is ADR-0071 §5's deferred fixpoint.
