...
**Pointer arithmetic exists, and a bump allocator is finally writable.** `p + n`, `n + p` and `p - n`
on a `*T` now work, element-scaled (`p + 1` on a `*s64` advances eight bytes, ADR-0064 §2) and
unchecked (a raw pointer has no length to check, §3). A program can carve a region from one `malloc`
and hand out cells by advancing a pointer — the shape temporary storage wants, and the thing
`recording_alloc` in `050` could count bytes for but not do. **919 workspace tests**, all six gates
green, plus **156 Neovim checks** verified rather than gated. See §1.5.

**The wave was scoped by finding out how it lowers, and narrowing when the lowering said so.** The
first draft included `p - q`, the pointer difference. Working out the lowering showed it needs the
element stride to divide by — and the stride is layout, which ADR-0017 §5 keeps out of `jr-mir`
entirely (the back ends scale a `Projection::Index`; the mid-end never sees a size). So `p - q` would
need a new MIR node or a layout query `jr-mir` lacks, which a bump allocator (only ever *advancing* a
pointer) does not force. It was cut mid-wave and deferred (ADR-0064 §5), keeping the "no new MIR node"
property true for what landed.

**No new MIR node, no back-end change.** `p + n` lowers to the address of a slot holding the pointer,
indexed by `n` — the same load-then-scale a view's `data` word takes, so both back ends scale by the
element stride and no size appears in `jr-mir`. `p - n` negates the offset first. The `valid/052`
snapshot shows `store s1 <- v1` then `addr s1[v2]`.
...
### What the pointer-arithmetic wave landed

- [x] **ADR-0064**: `p + n`, `n + p`, `p - n` on `*T`, element-scaled and unchecked, lowered to an
      indexed address both back ends already scale. **No new diagnostic code** (E0258 still first free)
      and **no new MIR node** — the type rules are new `check_binary` arms and the lowering builds an
      indexed address.
- [x] **`p - q` is deferred, on purpose** (ADR-0064 §5): its element-count result needs the stride,
      which is layout `jr-mir` does not carry. Cut mid-wave rather than smuggled in with a layout query
      the mid-end lacks. It is E0223 until its own wave.
- [x] **A regression the differential caught**: typing operands with no expectation broke
      `sum: s64 = xx tiny + 1;` — the autocast lost its `s64` context and became E0242. The fix is a
      guard that skips the pointer probe when a concrete numeric type is expected (a pointer could
      never satisfy an `s64` annotation anyway).
- [x] **The refusals are specific**: `p - q` and `n - p` and a non-integer offset are all E0223, but
      each names its own mistake — "cannot subtract a pointer from `s64`" points at reversed operands.

Two things worth carrying forward.

- **How a feature lowers can narrow its scope, and finding that out early is the point.** `p - q` read
  as obviously part of "pointer arithmetic" until the lowering showed it needs layout the mid-end does
  not have. Scoping by writing the lowering — not by the feature's name — is what caught it before an
  ADR promised it.
- **A raw pointer is indexed by spilling it to a slot**, because `Projection::Index` scales only when
  the place's type at that step is a pointer, and a pointer *value* is not in memory. That is the same
  path a view's `data` word takes — reused, not reinvented, which is why neither back end changed.
...
- [ ] **Temporary storage is W3's last feature**, and its blocker is gone: a bump allocator is now
      writable (`p + n`). What remains is the arena itself — a scratch region the context points at,
      reset at a known point — and `push_context` (ADR-0063) already gives it the scoping. This is the
      next wave to close W3's data structures.
- [ ] **Traps with backtraces** remains the other W3 feature, and it is independent of everything
      ...
      representation in both engines. Its native half is the hard part (a stack walk entangled with the
      inliner), which is why it is not the obvious next pick despite being unblocked.
