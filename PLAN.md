...
**An array length may name a constant, and W4's second sub-wave found its scheduled work already done.**
`[N]s64` with `N :: 4` now resolves; `modules/Basic`'s owed `[20]u8` buffer can name its size. **930
workspace tests**, all six gates green, plus **166 Neovim checks** verified rather than gated. See §1.5.

**Sub-wave 2 was "aggressive const folding", and const-prop already does it.** ADR-0069 §4 and this
section both said a `#run` result "reaches the body as a constant, but the arithmetic around it is not
re-folded". That is true of the **built** MIR — which is what the corpus snapshots, and so what the claim
was looking at — and false of the **optimized** MIR, which is what both engines consume:

```text
built:      v1: s64 = 5_s64 * 10_s64
optimized:  Return(Some(Constant(...)))     // 57, one constant, no arithmetic left
```

`exit(m)` on that program exits 57. ADR-0022's const-prop folds through a `#run` result exactly as
through any other constant, because by then the `#run` *is* one. Seeing it took probing the optimized
body; nothing the tests display would have shown it. **This is the second time this session a scheduled
dependency turned out not to exist** — ADR-0067 §0 was the first — which makes it a pattern rather than a
coincidence: a plan's stated reason is checkable, and checking it costs minutes.

So the sub-wave was rescoped to a gap that is real, and ADR-0070 amends ADR-0039 §3a: its argument
("sema cannot ask for `COUNT`'s value without inverting that dependency") is right about a length needing
**evaluation** and too broad about constants. For `N :: 4` nothing needs evaluating.
...
### What the array-length wave landed

- [x] **ADR-0070**: `[N]T` where `N` names a literal-valued constant, resolved in sema from the HIR with
      no evaluation. **No new diagnostic code, no new pool item, no MIR change** — the MIR shows `[4]s64`,
      so nothing downstream can tell how the length was written, which is the evidence it belongs in type
      resolution. **E0261 is still the first free code.**
- [x] **The line is "already a literal, one name away"**, not "is a literal" (§2). Arithmetic, a `#run`,
      a chain of constants and a cross-file constant all still need the evaluator and stay refused —
      folding them here would mean a *second* constant folder beside ADR-0022's, the duplication
      ADR-0018 §2 refuses for layout and ADR-0020 §2 for trap messages.
- [x] **E0233's message was false after the change** and is reworded: "an array length must be an integer
      literal" stopped being true, so it now names what was found on each side of the line.
- [x] **A type-errors file stopped being an error**, because `023-array-length-not-literal.jr` used
      exactly `COUNT :: 4`. Rewritten around arithmetic — a case refused by the *phase order* rather than
      by an unimplemented feature.

Two things worth carrying forward.

- **Check the query a claim is about.** "Not folded" was true of the built MIR and false of the optimized
  one, and the corpus only ever displays the built one. A claim about optimisation needs a probe of the
  optimized body; the snapshots cannot see it.
- **A too-broad argument in an ADR outlives the wave that wrote it.** ADR-0039 §3a bundled "needs
  evaluation" with "is not a literal" and refused both together for one reason that only covered the
  first. That stood for thirty ADRs. Amending it needed no new machinery — only noticing that the reason
  did not reach as far as the rule.
...
- [ ] **W4 sub-wave 3 is RTTI**, and it is the next thing — `Type` values, `type_info()`, `Any`. It is the
      largest of the four and the one §5 means by "sema and the VM become mutually recursive"; the named
      deliverable is **cycle detection with readable errors**, not the features. Three refusals are waiting
      on it and will come free: an array length that needs evaluation (ADR-0070 §2), an operator overload
      or a default argument in a `#run`, and a `#run` reading another file's constant.
