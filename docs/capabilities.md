...
- **328** `.jr` corpus files under `tests/corpus/` outside `tests/corpus/modules/`
  (**342** counting those).
- **245** accepted ADRs — see [`docs/adr/README.md`](adr/README.md).
...
| Control a struct's layout | `x: s64 #align 8 * 2;`, `y: s64 #place BASE + 16;` | Raise a field's alignment, or put it at an exact byte offset (ADR-0144, ADR-0245). `#align` is a *minimum*, a power of two up to 4096; `#place` takes any non-negative offset, may be unaligned, and **may overlap another field** — that is the point, and nothing checks for it, exactly as an untagged `union` reinterprets bits. A placed field never moves the ones after it. Local integer arithmetic and alias chains are evaluated before layout; imported constants and procedure calls remain deferred |
...
| Fold a compile-time call | `COMPUTED :: #run add(2, b = 3)`, or `n := #run add(2)` in a body when `b` defaults | Nested calls, arithmetic around a call, loops, **imported** callees, literal defaults and named reordering all work (ADR-0069, ADR-0234). Still refused: a `#foreign` call (ADR-0006), an operator overload written directly in the thunk expression, and reading another file's constant |
...
| `struct { … }`, one level, nominal, with `#soa(N)` for one-array-per-field storage and `e[i].x` access (ADR-0147), and per-field `#align N` (a minimum, power of two up to 4096) and `#place N` (an exact byte offset, may overlap and may be unaligned) — ADR-0144. Local integer expression graphs may supply all three operands (ADR-0245) | a struct-level `#align`; any packing form; `#align` on a local or a procedure; an operand requiring an imported constant or procedure call; a bare `e[i]` on an `#soa` struct, and `using` inside one |
...
| `enum { RED; GREEN :: 2 + 3; }`, nominal, namespaced members, and bare `.RED` from context — including as a `switch` case (ADR-0067). A member's value may use a local integer expression or alias chain, and auto-numbering continues from it (ADR-0129, ADR-0245) | a value requiring a procedure call/`#run` or another file's constant; a member naming a **sibling** member |
...
| named arguments `f(b = 2, a = 1)`, explicit literal defaults `(b: s64 = 10)`, and inferred literal defaults `(b := 10)`, including ordinary local/imported procedures and their `#run` calls. Pure `$T`, local `$N`, and local mixed `$T`+`$N` calls use the same declaration-ordered binder; a literal may default `$N` itself, while supplied arguments—not defaults—must pin every type variable (ADR-0236, ADR-0241). Inferred `int`/`#char`, float, `bool`, and string defaults fix the parameter type to `s64`, `float64`, `bool`, and `string` respectively | a non-literal default; `null` without an explicit pointer type (E0257); a default whose type contains `$T`; imported `$N`/mixed calls (E0268); a pure-template call inside `#run`; parameter-name/default metadata through a procedure pointer |
...
| **`$T` polymorphic procedures** — inferred from arguments directly, through pointers/views/vectors/dynamic arrays, and through parameterised nominal instances such as `*Table($K, $V)` (ADR-0081–0084, ADR-0229); instantiated once per bound-type tuple and checked/run as ordinary procedures. Pure `$T` and local mixed `$T`+`$N` calls share ordinary named/default binding for fixed-type parameters (ADR-0236, ADR-0241). A template may call another template (ADR-0120), and a pure `$T` template imported from another module is specialised in its declaration file for the root program (ADR-0230) | two-way unification and explicit type arguments; an imported `$N` or mixed `$T`+`$N` instantiation (E0268); an imported polymorphic `#expand` (E0272) |
...
| **`$N` comptime-value parameters and local instantiation** — named arguments, fixed literal defaults, and defaults on `$N` itself produce declaration-ordered specialization keys; equivalent omitted/explicit values share one clone (ADR-0087, ADR-0088, ADR-0241). Mixed local `$T`+`$N` templates use the same binder | a non-constant argument (E0271); imported `$N` or mixed templates (E0268); a pure-template call inside `#run` |
...
| Sema (signatures, checking, inference) | **Works** | A union's diagnostics are a struct's unchanged, deliberately. Final signatures consume VM-evaluated local integer expression graphs for array/SIMD counts, enum values, layout attributes and SoA, while keeping `$N`'s instantiation path; imported and callable declaration expressions remain deferred (ADR-0245). Float literals are context-typed with **no** fit check, because IEEE-754 saturates (ADR-0040 §5) |
...
- **An array length may use a local evaluated integer expression.** `[20]u8`, `[COUNT]u8`,
  `[WIDTH * HEIGHT]T`, local alias chains, enum members, and `[N]T` under a bare `$N` parameter
  work. The pre-signature pass uses the ordinary MIR/VM semantics. An imported constant, a
  procedure call/`#run`, a count depending on arithmetic over `$N`, or a declaration synthesized
  by computed `#insert` remains deferred (ADR-0245).
