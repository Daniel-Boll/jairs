...
| **Configure the formatter** | `[fmt] indent_style`, `indent_width` in `jairs.toml` | Spaces or **tabs**, and any width. Honoured by `jr fmt` *and* over LSP, so the setting is not one that works only from a terminal. An unrecognised key is an **error**, not ignored — which is also why there is no `max_width`: it was declared and defaulted for waves and never read, so offering it would have been a setting that appears to work. Line wrapping is genuinely unimplemented (ADR-0202 §2) |
