...
- **282** `.jr` corpus files under `tests/corpus/` outside `tests/corpus/modules/`
  (**290** counting those).
- **206** accepted ADRs — see [`docs/adr/README.md`](adr/README.md).
...
| **Configure the formatter** | `[fmt] indent_style`, `indent_width`, `max_width` in `jairs.toml` | Spaces or **tabs**, and any width. Honoured by `jr fmt` *and* over LSP, so the setting is not one that works only from a terminal. An unrecognised key is an **error**, not ignored. `max_width` was absent for exactly that reason until wrapping existed — declared and defaulted for waves, never read. It breaks a call's argument list and a procedure's parameter list; it **never** reflows a comment, does not break a boolean chain, and cannot break a string literal, so a formatted file may still hold a longer line. That scope came from counting the corpus — 3436 of 3472 over-width lines are comments and only 25 are code (ADR-0204) |
...
| Compile to a native executable | `jr build file.jr -o out` | arm64 macOS verified. **x86-64 Linux read at last — six times — and it was failing** (ADR-0205): six defects, starting with `modules/Math` binding the math routines to **libc**, which macOS resolves (`libm.tbd` is a symlink to `libSystem.tbd`) and glibc does not, since libm is a separate library there. The differential harness now agrees across both engines on all 138 corpus programs there. The last failure was a **silent miscompile**: a 32-byte four-`float64` struct — a `CGRect` — went in four SSE registers where System V requires the stack, because the classification had AAPCS64's homogeneous-aggregate rule and System V has none. Fixed in ADR-0206, splitting ADR-0160's refusal as that ADR predicted the Linux run would allow. The claim had been "unverified" for waves while the answer sat unread in CI. A declared `BUILD_OUTPUT` is confined to the working directory (ADR-0122) |
