...
| named arguments `f(b = 2, a = 1)`, explicit literal defaults `(b: s64 = 10)`, and inferred literal defaults `(b := 10)`, including ordinary local/imported procedures and `#run`. Inferred `int`/`#char`, float, `bool`, and string defaults fix the parameter type to `s64`, `float64`, `bool`, and `string` respectively | a non-literal default; `null` without an explicit pointer type (E0257); inferred defaults on `$T`/`$N`/`$$T` template/comptime procedures (E0252); parameter-name/default metadata through a procedure pointer |
