...
- **279** `.jr` corpus files under `tests/corpus/` outside `tests/corpus/modules/`
  (**290** counting those).
- **196** accepted ADRs — see [`docs/adr/README.md`](adr/README.md).
...
| **Build a project with a Jairs program** — `jr build build.jr`, no flag. The script reads its command line, shells out (`git rev-parse`, a shader compiler, `codesign`), chooses per OS, generates sources, and compiles targets at a chosen optimisation level | ADR-0195, ADR-0196; `examples/10-build-script.jr` and `examples/11-run-build-script.jr` | Two spellings. A **`main`** script is run by the driver and gets `Compiler.build` (immediate, returns a `bool`) plus shelling out. A **`#run`** script — Jai's shape, no `main` — can allocate, print and declare targets with `Compiler.request_build`, but cannot compile from inside itself (salsa: "Cannot change database mid-query") and cannot shell out, since that reaches a host and `#foreign_at_comptime` does not exist. Neither spelling reacts to a target's diagnostics: with no message loop there is nothing to interleave |
