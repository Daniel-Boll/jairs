...
- **279** `.jr` corpus files under `tests/corpus/` outside `tests/corpus/modules/`
  (**290** counting those).
- **195** accepted ADRs — see [`docs/adr/README.md`](adr/README.md).
...
| **Build a project with a Jairs program** — `jr build build.jr`, no flag. The script reads its command line, shells out (`git rev-parse`, a shader compiler, `codesign`), chooses per OS, generates sources, and compiles targets at a chosen optimisation level | ADR-0195; `examples/10-build-script.jr` | It is a *program*, not a `#run` — comptime code may call no `#foreign`, so a `#run` cannot read a file or allocate. Shelling out goes through the driver, because the VM marshals a pointer one level deep and no type distinguishes `argv` from an out-parameter. The script cannot react to a target's diagnostics: it gets a `bool`, since with no message loop there is nothing to interleave |
