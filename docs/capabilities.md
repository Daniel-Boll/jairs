...
- **323** `.jr` corpus files under `tests/corpus/` outside `tests/corpus/modules/`
  (**337** counting those).
- **240** accepted ADRs — see [`docs/adr/README.md`](adr/README.md).
...
| named arguments `f(b = 2, a = 1)`, explicit literal defaults `(b: s64 = 10)`, and inferred literal defaults `(b := 10)`, including ordinary local/imported procedures and their `#run` calls. An ordinary call to a pure `$T` procedure may use either default form on a fixed-type parameter; supplied arguments, not defaults, must pin every type variable (ADR-0236). Inferred `int`/`#char`, float, `bool`, and string defaults fix the parameter type to `s64`, `float64`, `bool`, and `string` respectively | a non-literal default; `null` without an explicit pointer type (E0257); a default whose type contains `$T`, or a default/local named call on a `$N` or mixed `$T`+`$N` procedure (E0252); imported `$N`/mixed calls (E0268); a pure-template call inside `#run`; parameter-name/default metadata through a procedure pointer |
...
| **`$T` polymorphic procedures** — inferred from arguments directly, through pointers/views/vectors/dynamic arrays, and through parameterised nominal instances such as `*Table($K, $V)` (ADR-0081–0084, ADR-0229); instantiated once per bound-type tuple and checked/run as ordinary procedures. Pure `$T` calls share ordinary named/default binding for fixed-type parameters (ADR-0236). A template may call another template (ADR-0120), and a pure `$T` template imported from another module is specialised in its declaration file for the root program (ADR-0230) | two-way unification and explicit type arguments; an imported `$N` or mixed `$T`+`$N` instantiation (E0268); an imported polymorphic `#expand` (E0272) |
