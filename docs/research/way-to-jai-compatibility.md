...
| 17 — Procedures | Defaults/named args, multiple returns, `#must`, overloads, inline, `#this`, reflection, anonymous procs | **Partial** | Local/nested procedures, procedure values, literal defaults, named arguments, several returns, declaration-only labels such as `-> (value: s64, found: bool)`, `#must`, recursion, and operator overloading exist. General procedure overloading, unparenthesized/defaulted named results, implicit result bindings, anonymous/lambda procedures, `inline`, `#this`, `#procedure_name`, `#deprecated`, and full procedure reflection do not. |
