...
| 10 — Pointers | Address, dereference, pointer chains, null, casts | **Partial** | Typed pointers, `null`, address/deref, element-scaled offsets and same-type signed pointer difference, `typed/untyped`, and FFI pointers exist. Indexing sugar, ordering, and general pointer casts are absent or deliberately constrained. |
...
| 22 — Polymorphic procedures | `$T`, lambdas, proc arguments, `#bake_arguments`; [`examples/22/22.6_baked_args.jai`](../../references/The_Way_to_Jai/examples/22/22.6_baked_args.jai) | **Partial** | `$T`, multiple type variables, `$N`, procedure arguments, template calls, instantiation, `$$T`, literal `#bake_arguments`, and named/fixed-default arguments on pure `$T` calls exist. Lambda `=>`, explicit type arguments, broad two-way inference, cross-file `$N`/mixed instantiation, defaults on comptime templates, and several higher-order forms do not. |
