...
; disagree about more than spelling. Neovim uses `@indent.begin`, `@indent.end` and
; `@indent.branch`; Zed's `@indent` names a *range* and `@outdent` ends the innermost one at the
; start of the captured node, so translating token-for-token would indent one level too many at
; every closer.
;
