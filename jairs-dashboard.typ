...
#h(4pt)
#pill(fill: rgb("#fdf2e6"), stroke: warn)[W7 OPEN · String, Sort]

#v(0.5em)
#grid(
  columns: (1fr, 1fr, 1fr, 1fr, 1fr),
  gutter: 8pt,
  metric("Tests", "984", "workspace, all passing"),
  metric("Corpus", "193", "jr files, both engines"),
  metric("ADRs", "105", "0001 to 0105, immutable"),
...
#let waves = (
  ...
  (
    ...
    "OPEN, two modules shipped, and both were named by a refusal rather than by a checklist. String (ADR-0103) is equal, compare, starts_with, ends_with, find, contains, byte_at, is_empty — NONE of which allocate. It exists because ADR-0099 refused == on two strings: a string is {data, count}, so same storage and same contents are both plausible readings, and picking one silently would make the other a bug that looks like working code. That refusal own reasoning named the fix, comparing contents needs a byte loop, which is a library job rather than an operator — an == that looped would also be the only implicitly-looping operator in the language. So equal(a, b) is what E0278 help was pointing at. It is its OWN module rather than more of Basic, and the deciding argument is not size: Basic is imported by every program so anything in it taxes all of them, but more importantly adding to Basic would mean nothing ever tested that TWO modules can be imported at once, which every module test to date failed to do. Nothing allocates deliberately: concat and friends need somewhere to put a result, and while the mechanism exists (context.allocator, temporary storage) the CHOICE between always-the-context-allocator, an explicit parameter, and always-temporary does not — and settling it in passing is how a library acquires an accidental convention. byte_at exists because s.data[i] does not compile, a *u8 not being indexable, so reading a byte takes pointer arithmetic and a cast; out of range answers -1 rather than trapping, unlike an array index, because an array bound is known to the compiler so passing it is a mistake while scanning until the bytes run out is an ordinary loop. Sort (ADR-0104) is the second and the first POLYMORPHIC library code, so the first that depends on W5 rather than coexisting with it. Three language facts were probed before a line was written and all hold: a view parameter is mutable through the callee, a $T infers through a view, and a procedure pointer can be passed and called. The CALLER supplies the comparison, and that is a language fact rather than a taste: resolving an operator inside a $T template against the instantiated type is a lookup instantiation does not do — operator < exists and #modify can REJECT an instantiation, but nothing can SELECT an implementation per instantiated type, which would be operator-bounded polymorphism and belongs to whichever wave decides how a template states its requirements. Insertion sort, O(n squared) said plainly: STABLE which quicksort is not, needs NO storage where a merge sort would allocate and allocation is exactly what String declined to decide, and short enough to read — which matters for the first sorting routine in a language whose suite compares two independent engines. A faster algorithm is W8 with a benchmark behind it. Writing Sort found TWO leaked internal errors, which is the argument for a standard library written in the language paying out twice in one sub-wave. An imported procedure used as a VALUE reported this compiler has a gap please report it, for a legal program — the value was representable all along since ImportedProcs had resolved it to a ProcRef and a DeclId carries a file id, so what was missing was a three-line bridge; the local arm own comment even said a cross-file one is refused by that arm, so the refusal was KNOWN and undocumented, recorded as intended in a comment while users saw a bug report. And a call to an imported TEMPLATE leaked no routine for file 2 proc 0: cross-file instantiation is deferred, but the refusal did not exist because a $T parameter type is ERROR and ERROR matches anything, so the call type-checked. It is now E0268, carried across the boundary by template_names shaped exactly like macro_names, and the diagnostic NAMES THE WORKAROUND with a corpus file checking that the workaround works — a refusal is only as good as its escape route. It survived because nothing in the corpus had ever imported a polymorphic procedure. Both bugs were hiding behind a stale comment that said something checkable nobody had checked. So Sort sort_ints and ints_sorted are NOT conveniences today: they are the only way an importer can use a polymorphic module. Remaining in W7: the allocating half of String once the allocator convention is decided, a merge sort and binary search, a dynamic array and hash table which both want W5 polymorphic structs, Math, Random, File, Process, Thread, Time, Socket, JSON, Compiler.",
  ),
  (
...
#grid(
  ...
  [
    ...
    #v(0.3em)
    #text(size: 7.4pt)[
      Test count 900 to 984. Corpus 116 to 194 files. Neovim checks 103 to 166. *W2, W3, W4.5, W4 and W5 are all
