...
p[2] = value;      // unchecked access to the third element
p += 2;            // advance the pointer itself by two elements
```
...
Raw-pointer indexing `p[n]` is the place `(p + n).*`; it has no bounds check because `*T`
stores no length. `p += n` and `p -= n` use the same element-scaled offset. Pointer ordering
(`<`, `>`) remains <span class="jairs-status absent">absent</span>. Arithmetic and raw indexing
are unchecked — running past an allocation, reading or writing an invalid place, subtracting
unrelated pointers, or producing a non-element-aligned/unrepresentable difference is undefined
behaviour.
