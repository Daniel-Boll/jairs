...
- **Exhaustiveness over an enum, and `else` for anything else.** `describe` covers every `Colour` member and needs no `else`; omitting a member would be a compile error (E0258), whose editor quick fix adds every explicit missing arm rather than hiding future additions behind `else`. Adding a redundant `else` there would be E0260 since it could never run. But an `s64` has no finite member set to be exhaustive over, so `classify` uses `else` as the catch-all to make itself total.
