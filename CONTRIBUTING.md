...
- Use `scripts/check fast` while iterating and `scripts/check pre-commit` for
  broad local feedback. Both require `cargo-nextest`, use an isolated cache,
  and deliberately omit expensive exhaustive sweeps. `scripts/check measure`
  writes per-test JUnit timings. None replaces `scripts/check full`, the
  authoritative Cargo/libtest gate including doctests.
